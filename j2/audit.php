...
foreach ( $files as $file ) {
	...
	foreach ( $m as $hook ) {
		...
		/*
		 * A handler may guard itself, or call one method that does. Following
		 * exactly one level and then insisting the callee holds a real
		 * capability test keeps the delegation from becoming a way to pass by
		 * naming a method something reassuring.
		 */
		$checked = $body;

		if ( preg_match( '/\$this->(can_manage_ticket|verify|guard|assert)\w*\s*\(/', $body, $delegate ) ) {
			$checked .= method_body( $src, $delegate[1] );
		}

		$guarded = str_contains( $checked, 'current_user_can' ) || str_contains( $checked, 'is_user_logged_in' );
