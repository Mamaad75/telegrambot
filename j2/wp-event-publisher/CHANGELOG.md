...
نسخهٔ رابط کاربری و موبایل. تمرکز روی چیزی که روی گوشی دیده می‌شد: صفحهٔ تیکت که هیچ متنی و هیچ آیکنی نداشت، بدجی که واقعی نبود، رنگ‌های المنتور که کار نمی‌کردند، و دکمهٔ اعلانی که هیچ کاری نمی‌کرد.

---

## ۱. آیکن‌ها اصلاً بارگذاری نمی‌شدند — علت اصلی چیزی که در گوشی می‌دیدید

تمام آیکن‌های سمت کاربر با **Dashicons** کشیده می‌شدند. Dashicons فونت آیکن پیشخوان وردپرس است و وردپرس آن را در سمت کاربر بارگذاری **نمی‌کند** مگر افزونه بخواهد — و این افزونه هرگز نخواسته بود.

نتیجه: هر `<span class="dashicons …>` یک جعبهٔ خالی بود. آیکن فیلترها، دکمهٔ تیکت جدید، دکمهٔ ثبت، دکمهٔ اعلان، آیکن فرم، گیرهٔ پیوست — همه نامرئی.

حالا همهٔ آیکن‌های سمت کاربر **SVG درون‌خطی** هستند: فونتی برای گم شدن ندارند، رنگشان را از عنصر اطراف می‌گیرند و بدون فایل فونت مقیاس می‌شوند. (پنل‌های اطلاعیه و هاب که هنوز از Dashicons استفاده می‌کنند، حالا آن را صریحاً درخواست می‌کنند.)

## ۲. متن دسته‌بندی‌ها روی گوشی پنهان شده بود

یک قانون در فایل استایل، **زیر ۴۲۰ پیکسل برچسب فیلترها را `display:none`** می‌کرد. کنار علت اول، هر پنج فیلتر روی گوشی فقط یک «۰» نشان می‌دادند — دقیقاً همان تصویری که فرستادید.

حالا برچسب هرگز پنهان نمی‌شود. ردیف فیلتر یک نوار افقی قابل اسکرول با snap است: آیکن + متن + عدد، همیشه هر سه.

## ۳. فایل استایل شش بار روی خودش نوشته شده بود

`.jarchi-ticket-filter` **شش تعریف متناقض** داشت که در نسخه‌های پیاپی به انتهای فایل اضافه شده بودند. اینکه کدام برنده شود به جای قرار گرفتنشان در فایل بستگی داشت. یکی از آن‌ها `min-width:128px` می‌داد که برای پنج فیلتر یعنی ۶۴۰ پیکسل روی صفحهٔ ۳۹۰ پیکسلی.

بخش سمت کاربر از نو نوشته شد: **هر انتخابگر فقط یک بار**. دو قاعده برای هر چیزی که بعداً اضافه شود:

1. **قالب حریف است.** این نشانه‌گذاری داخل محتوای صفحهٔ کسی دیگر می‌نشیند، جایی که `.entry-content h2{font-size:2.6rem}` عادی است و سربرگ را به سه خط روی‌هم تبدیل می‌کند. هر چیزی که روی چیدمان اثر دارد صریح نوشته می‌شود، نه به ارث برده و امید بسته.
2. **اول گوشی.** چیدمان دسکتاپ افزوده است، از ۹۰۰ پیکسل به بالا.

## ۴. ریسپانسیو، واقعاً

- تک‌ستونی به‌صورت پیش‌فرض؛ ستون دوم از ۹۰۰ پیکسل اضافه می‌شود.
- **نوار پایین اپ دیگر روی کارت آخر نمی‌نشیند:** `env(safe-area-inset-bottom)` روی خود ظرف و روی جعبهٔ پاسخ چسبان.
- فیلدها **۱۶ پیکسل**‌اند — کمتر از آن و سافاری آیفون هنگام فوکوس صفحه را زوم می‌کند و کاربر را افقی می‌برد جایی که فرم را نمی‌بیند.
- هدف‌های لمسی حداقل ۴۴ و آیتم‌های لیست ۵۶ پیکسل.
- روی گوشی باز کردن تیکت، فهرست را کنار می‌گذارد و لینک بازگشت برمی‌گرداند؛ جعبهٔ پاسخ چسبیده به پایین می‌ماند.

## ۵. عددها واقعی شدند

- **بدج سربرگ:** بدون کلاسی که نمایشش می‌داد رندر می‌شد و منتظر اولین پیمایش می‌ماند — یعنی صفحه بدون بدج باز می‌شد و ده ثانیه بعد یکی ظاهر می‌شد. حالا وضعیتش سمت سرور تصمیم گرفته می‌شود. و حالا **لینک** است، نه برچسب: می‌زنید و تیکت‌ها را می‌آورد.
- **شمارش فیلترها بعد از فیلتر محاسبه می‌شد.** با انتخاب «پاسخ داده شده»، «همه» عدد پاسخ‌داده‌شده را نشان می‌داد و بقیه صفر — ردیف فیلتر دقیقاً در همان لحظه‌ای که به کار می‌آید از توصیف تیکت‌ها دست می‌کشید. حالا شمارش قبل از فیلتر گرفته می‌شود.
- **بدج در منوی سایت:** هر آیتم منویی که به صفحهٔ تیکت اشاره کند حالا خودکار بدج زنده می‌گیرد. نیازی به قرار دادن شورت‌کد در جایی که جا نمی‌شود نیست. مقایسه روی **مسیر** انجام می‌شود، پس اسلش انتهایی، لینک نسبی و دامنهٔ متفاوت همه پیدا می‌شوند. همان `data-jarchi-ticket-badge` است که اسکریپت پیمایش رنگ می‌زند، پس عدد منو و عدد صفحه هرگز نمی‌توانند با هم اختلاف داشته باشند.

## ۶. المنتور — چرا تغییر رنگ هیچ کاری نمی‌کرد

هر کنترل رنگ از `sanitize_hex_color()` رد می‌شد که فقط `#RGB` و `#RRGGBB` را می‌پذیرد. المنتور به‌طور معمول چهار چیز دیگر برمی‌گرداند:

| مقدار | چه وقت |
|---|---|
| `rgba(232, 79, 1, .6)` | هر بار که اسلایدر شفافیت لمس شود |
| `#E84F01AA` | همان |
| `hsl(20 97% 46%)` | تب دیگر انتخابگر |
| `var(--e-global-color-primary)` | **کلیک روی رنگ‌های سراسری — یعنی ردیف اول انتخابگر** |

هر چهار تا `null` برمی‌گشتند، انتخاب دور ریخته می‌شد و پیش‌فرض جایش می‌نشست. یعنی رنگ عوض می‌کردید و هیچ اتفاقی نمی‌افتاد. **این دقیقاً همان باگ بود.**

حالا یک اعتبارسنج مشترک همهٔ این شکل‌ها را می‌پذیرد. چون مقدار داخل یک اعلان CSS می‌نشیند، خطر خروج از آن اعلان است — پس به‌جای فهرست‌کردن راه‌های خروج، مقدار باید **کاملاً** با یکی از شکل‌های مجاز بخواند؛ هر چیز دیگری رد می‌شود. نقطه‌ویرگول و آکولاد در هیچ‌کدام از آن شکل‌ها ممکن نیست.

همچنین: قواعد سراسری المنتور دیگر رنگ ثابت با `!important` نمی‌نویسند — این کار خودش جلوی اثر کردن تنظیم هر نمونه را می‌گرفت. حالا همه از توکن‌های همان نمونه می‌خوانند.

## ۷. دکمهٔ فعال‌سازی اعلان

**هر** شاخهٔ شکست بی‌صدا `return false` می‌کرد. دکمه از یک دکمهٔ مرده قابل تشخیص نبود: می‌زدید و صفحه تکان نمی‌خورد — چه مرورگر رد کرده باشد، چه سایت کلید نداشته باشد، چه موفق شده باشد.

حالا هر نتیجه پیام خودش را دارد:

- **آیفون/آیپد:** «ابتدا سایت را به صفحهٔ اصلی اضافه کنید: Share ← Add to Home Screen، سپس از همان آیکن وارد شوید.» iOS تا قبل از نصب روی صفحهٔ اصلی وب‌پوش تحویل نمی‌دهد — این محدودیتی نیست که کاربر بتواند منتظرش بماند، کاری است که می‌تواند انجام دهد.
- اجازه قبلاً مسدود شده / کاربر رد کرد / سایت HTTPS نیست / کلید VAPID تنظیم نشده / ذخیره روی سرور شکست خورد / موفق شد — هرکدام پیام روشن فارسی.
- **موفقیت را پاسخ سرور تعیین می‌کند**، نه اشتراک مرورگر. قبلاً «فعال شد» برای اشتراکی گزارش می‌شد که سایت هرگز ذخیره‌اش نکرده بود.
- دکمه فقط جایی نشان داده می‌شود که زدنش به جایی برسد. دعوتی که نمی‌شود پذیرفت، از پیشنهاد ندادن بدتر است.
- اسکریپت `defer` است، پس ممکن است بعد از `DOMContentLoaded` اجرا شود — انتظار برای رویدادی که گذشته یعنی انتظار برای همیشه. حالا هر دو حالت را می‌گیرد.

---

## آزمون‌ها

پنج مجموعه، همه سبز. **۱۰ جهش** روی همین اصلاحات اجرا شد — هر محافظ یک‌به‌یک برداشته شد و بررسی شد آزمون واقعاً شکست می‌خورد. چهار آزمون در اولین دور جهش **زنده ماندند** (چیزی را چک می‌کردند که مستقل از باگ درست بود) و سفت شدند تا واقعاً بگیرند.

**آنچه ادعا نمی‌شود:** این‌ها روی بدل آزمایشی اجرا می‌شوند. رندر واقعی در یک مرورگر واقعی روی یک گوشی واقعی آزمون نشده؛ نشانه‌گذاری، قواعد CSS و منطق جاوااسکریپت آزمون شده‌اند.
