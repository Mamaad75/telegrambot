...
final class TicketAutomations {
	...
	public function run_manual(): void {
		...
		foreach ( $this->rules() as $rule ) {
			if ( $id === (string) ( $rule['id'] ?? '' ) && $user_id > 0 ) {
				// A manual run ignores once_per_user: the administrator asked
				// for it explicitly, and refusing silently would look broken.
				$rule['once_per_user'] = false;
				$this->create_ticket( $rule, $user_id, array() );
				break;
			}
		}
	...
	public function scan_scheduled_rules(): void {
		...
		$users = get_users( array( 'number' => 500, 'fields' => 'ID' ) );
	...
	private function fire_rule( array $rule, int $user_id, array $context ): void {
		...
		$marker = $this->marker_key( $rule_id, $context );

		if ( ! empty( $rule['once_per_user'] ) && get_user_meta( $user_id, $marker, true ) ) {
			return;
		}
		...
		if ( $delay > 0 ) {
			wp_schedule_single_event( time() + ( $delay * MINUTE_IN_SECONDS ), 'jarchi_fire_automation_delayed', array( $rule, $user_id, $context ) );

			if ( ! empty( $rule['once_per_user'] ) ) {
				update_user_meta( $user_id, $marker, 'scheduled:' . time() );
			}

			return;
		}

		$this->create_ticket( $rule, $user_id, $context );
	...
	/**
	 * The user-meta key that records "this rule already ran".
	 *
	 * An order rule keyed only by rule id would send one message per customer
	 * ever, which is wrong — the second order deserves its own confirmation.
	 * Including the order makes "once" mean once per order.
	 *
	 * @since 1.9.0
	 *
	 ...
	 */
	private function marker_key( string $rule_id, array $context ): string {
		$suffix = '';

		if ( ! empty( $context['order_id'] ) ) {
			$suffix = '_o' . sanitize_key( (string) $context['order_id'] );
		} elseif ( ! empty( $context['post_id'] ) ) {
			$suffix = '_p' . absint( $context['post_id'] );
		}

		return '_jarchi_automation_' . $rule_id . $suffix;
	}
	...
	public function fire_delayed( $rule, $user_id = 0, $context = array() ): void {
		...
		$this->create_ticket( $rule, (int) $user_id, (array) $context );
	}
	 ...
	 * @return void
	 */
	private function create_ticket( array $rule, int $user_id, array $context ): void {
		...
		try {
			...
			if ( is_wp_error( $result ) && class_exists( Logger::class ) ) {
				wpep()->logger()->event(
					'ticket-automation',
