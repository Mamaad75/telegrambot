...
final class Tickets {
	...
	public function create_local_automated_ticket( int $customer_id, string $subject, string $body, array $options = array() ): int|\WP_Error {
		 ...
		 */
		update_post_meta( $ticket_id, '_jarchi_ticket_status', $allow_reply ? 'answered' : 'closed' );
	...
	public function run_auto_close(): void {
		...
		$ids=get_posts(array('post_type'=>self::POST_TYPE,'post_status'=>array('publish','private','draft'),'posts_per_page'=>200,'fields'=>'ids','meta_query'=>array(array('key'=>'_jarchi_ticket_status','value'=>array('waiting','reviewing'),'compare'=>'IN')),'date_query'=>array(array('column'=>'post_modified','before'=>gmdate('Y-m-d H:i:s',$cutoff)))));
	...
	private function handle_uploads(): array {
		...
		for ( $i = 0; $i < $count; $i++ ) {
			...
			if ( count( $ids ) >= self::MAX_ATTACHMENTS ) {
				...
				$errors[] = sprintf( __( 'حداکثر %d فایل پذیرفته می‌شود؛ بقیه نادیده گرفته شد.', 'wp-event-publisher' ), self::MAX_ATTACHMENTS );
				break;
			}

			// media_handle_upload() reads one entry, so the multi-file upload
			// is reshaped into the single-file form it expects.
			...
			$attachment_id = media_handle_upload( 'jarchi_attachment', 0 );
	...
	public function handle_assign_agent(): void {
		if ( ! current_user_can( Admin::CAPABILITY ) ) { wp_die( esc_html__( 'اجازه دسترسی ندارید.', 'wp-event-publisher' ) ); }
		check_admin_referer( self::NONCE ); $ticket_id = absint( $_POST['ticket_id'] ?? 0 ); $agent = absint( $_POST['agent'] ?? 0 ); $this->assign_agent( $ticket_id, $agent );
	...
	private function send_backend_ticket_event( string $type, int $ticket_id, array $extra = array() ): void {
		$ticket = get_post( $ticket_id ); if ( ! $ticket ) { return; } $user = get_user_by( 'id', (int) $ticket->post_author ); $terms = wp_get_post_terms( $ticket_id, self::TAXONOMY );
		$categories = wp_get_post_terms( $ticket_id, self::CATEGORY );
	...
	public function handle_submit(): void {
		...
		$this->insert_message( $ticket_id, $body, get_current_user_id(), 'user', 'waiting' );
	...
	public function handle_reply(): void {
		...
		$is_admin = current_user_can( Admin::CAPABILITY );
	...
	public function handle_status(): void {
		...
		if ( $ticket_id && in_array( $status, array( 'waiting', 'reviewing', 'answered', 'closed', 'open', 'pending' ), true ) ) {
	...
	private function store_attachments(): array {
		...
		$files = $_FILES['attachments'];
		$ids = array();
		$advanced=$this->advanced_settings(); $limit = min( max(1,(int)$advanced['max_files']), count( $files['name'] ) );
		for ( $i = 0; $i < $limit; $i++ ) {
			if ( empty( $files['name'][ $i ] ) || (int) $files['error'][ $i ] !== UPLOAD_ERR_OK ) {
				continue;
			}
			$type = wp_check_filetype( $files['name'][ $i ] );
			$advanced = $this->advanced_settings();
			$allowed_types = array( 'image/jpeg', 'image/png', 'image/webp', 'image/gif', 'application/pdf' );
			if ( ! empty( $advanced['allow_voice'] ) ) $allowed_types = array_merge( $allowed_types, array( 'audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/webm' ) );
			if ( empty( $type['type'] ) || ! in_array( $type['type'], $allowed_types, true ) ) {
				continue;
			}
			if ( (int) $files['size'][ $i ] > max(1,(int)$advanced['max_file_mb']) * MB_IN_BYTES ) {
				continue;
			}
	...
	private function front_new_ticket_markup( array $departments, array $categories, array $overrides = array() ): string {
		...
		$out .= '<div class="jarchi-ticket-upload"><input type="file" name="attachments[]" accept="image/jpeg,image/png,image/webp,image/gif,application/pdf,audio/mpeg,audio/wav,audio/ogg,audio/webm" multiple /><small>' . esc_html__( 'حداکثر ۴ تصویر، هر تصویر تا ۸ مگابایت.', 'wp-event-publisher' ) . '</small></div>';
