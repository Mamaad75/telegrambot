		...
		<table class="form-table" role="presentation">
			...
			<tr>
				...
				<td>
					...
					/>
					<p class="description">
						<?php esc_html_e( 'متن دکمه تماس.', 'wp-event-publisher' ); ?>
					</p>

					<fieldset class="wpep-contact-mode">
						<legend class="screen-reader-text"><?php esc_html_e( 'دکمه تماس به کجا وصل شود؟', 'wp-event-publisher' ); ?></legend>

						<p><strong><?php esc_html_e( 'دکمه تماس به کجا وصل شود؟', 'wp-event-publisher' ); ?></strong></p>

						<label>
							<input
								type="radio"
								name="<?php echo esc_attr( $wpep_name ); ?>[contact_mode]"
								value="author"
								<?php checked( 'support' !== ( $config['contact_mode'] ?? 'author' ) ); ?>
							/>
							<?php esc_html_e( 'شمارهٔ خود آگهی‌دهنده', 'wp-event-publisher' ); ?>
						</label>
						<p class="description">
							<?php esc_html_e( 'دکمه فقط روی آگهی‌هایی می‌آید که شمارهٔ تماسشان منتشر شده باشد. دکمه‌ای که شماره‌ای پشتش نباشد بن‌بست است، پس روی بقیه نمایش داده نمی‌شود.', 'wp-event-publisher' ); ?>
						</p>
						...
						<label>
							<input
								...
								<?php checked( 'support' === ( $config['contact_mode'] ?? 'author' ) ); ?>
							/>
							<?php esc_html_e( 'یک آیدی ثابت پشتیبانی — روی همهٔ آگهی‌ها', 'wp-event-publisher' ); ?>
						</label>
						...
						<p>
							<input
								...
								dir="ltr"
								name="<?php echo esc_attr( $wpep_name ); ?>[contact_target]"
								value="<?php echo esc_attr( (string) ( $config['contact_target'] ?? '' ) ); ?>"
								placeholder="@iranexim_support"
							/>
						</p>

						<p class="description">
							<?php esc_html_e( 'آیدی را با یا بدون @ بنویسید (مثلاً @iranexim_support). روی تلگرام به t.me و روی بله به ble.ir وصل می‌شود. اگر می‌خواهید جای دیگری برود — واتس‌اپ، فرم، صفحهٔ پشتیبانی — یک لینک کامل با https بگذارید.', 'wp-event-publisher' ); ?>
						</p>

						<p class="description">
							<?php esc_html_e( 'در این حالت دکمه روی همهٔ آگهی‌ها می‌آید، حتی آگهی‌هایی که شمارهٔ تماس ندارند.', 'wp-event-publisher' ); ?>
						</p>
					</fieldset>
				</td>
			</tr>
			...
			<tr>
				<th scope="row"><?php esc_html_e( 'نمایش شماره در متن', 'wp-event-publisher' ); ?></th>
				<td>
					<label class="wpep-switch">
						<input
							type="checkbox"
							name="<?php echo esc_attr( $wpep_name ); ?>[show_phone]"
							value="1"
							<?php checked( ! array_key_exists( 'show_phone', $config ) || ! empty( $config['show_phone'] ) ); ?>
						/>
						<span class="wpep-switch__track" aria-hidden="true"></span>
						<span class="wpep-switch__label"><?php esc_html_e( 'شمارهٔ تماس داخل متن پیام نوشته شود', 'wp-event-publisher' ); ?></span>
					</label>
					<p class="description">
						<?php esc_html_e( 'اگر خاموش باشد، شماره داخل متن آگهی نوشته نمی‌شود ولی دکمهٔ تماس همچنان کار می‌کند. برای وقتی که می‌خواهید تماس از مسیر پشتیبانی انجام شود و شماره روی کانال منتشر نشود.', 'wp-event-publisher' ); ?>
					</p>
					<p class="description">
						<?php esc_html_e( 'این گزینه فقط متن را کنترل می‌کند؛ اینکه اصلاً شماره‌ای منتشر بشود یا نه، همچنان به تنظیم فیلد شماره در «فیلدها» بستگی دارد.', 'wp-event-publisher' ); ?>
					</p>
				</td>
			</tr>
		</table>
