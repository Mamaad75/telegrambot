...
$wpep_notices = array(
	...
	'standard_enabled' => __( 'بسته استاندارد جارچی فعال و قوانین اصلی تیکت‌های خودکار همگام شد.', 'wp-event-publisher' ),
	'template_saved' => __( 'متن تیکت خودکار ذخیره شد.', 'wp-event-publisher' ),
	'not_found' => __( 'تیکت خودکار موردنظر پیدا نشد.', 'wp-event-publisher' ),
);
?>
<div class="wrap wpep-wrap wpep-dashboard wpep-inner-page jarchi-automations" dir="rtl">
	...
	<section class="wpep-card">
		...
		<form method="post" action="<?php echo esc_url( $wpep_post_url ); ?>" style="margin-top:14px">
			<?php wp_nonce_field( $nonce_action ); ?>
			<input type="hidden" name="action" value="wpep_ticket_automation_standard_pack" />
			<button type="submit" class="button button-primary"><?php esc_html_e( 'فعال‌سازی بسته استاندارد جارچی', 'wp-event-publisher' ); ?></button>
			<p class="description"><?php esc_html_e( 'قوانین اصلی جارچی را بدون نیاز به تنظیم فنی همگام می‌کند؛ از جمله پروفایل، آگهی، دیدگاه و نردبان. تکمیل پروفایل فقط هنگام تغییر واقعی از پروفایل ناقص به کامل ارسال می‌شود و هشدار قدیمی ۷ روز مانده به انقضای آگهی حذف شده است.', 'wp-event-publisher' ); ?></p>
		</form>
	</section>
		...
		<div class="jarchi-preset-grid">
			<?php foreach ( $presets as $wpep_preset ) : ?>
				...
				<article class="jarchi-preset<?php echo $wpep_used ? ' is-used' : ''; ?>">
					...
					<?php if ( ! empty( $wpep_preset['department']['name'] ) ) : ?>
						<p class="jarchi-preset__department"><span class="dashicons dashicons-groups" aria-hidden="true"></span><?php printf( esc_html__( 'ارسال از دپارتمان: %s — اگر وجود نداشته باشد خودکار ساخته می‌شود.', 'wp-event-publisher' ), esc_html( (string) $wpep_preset['department']['name'] ) ); ?></p>
					<?php endif; ?>
					...
					<footer class="jarchi-preset__foot">
						...
						<?php else : ?>
							<form method="post" action="<?php echo esc_url( $wpep_post_url ); ?>">
								...
								<?php if ( in_array( $wpep_trigger, array( 'post_published', 'post_unpublished' ), true ) ) : ?>
									<label class="wpep-field" style="margin:10px 0">
										<span><?php esc_html_e( 'نوع آگهی', 'wp-event-publisher' ); ?></span>
										<select name="post_type">
											<option value="<?php echo esc_attr( WPEventPublisher\TicketAutomations::POST_TYPE_CUSTOM_PUBLIC ); ?>"><?php esc_html_e( 'همه CPTهای سفارشی عمومی', 'wp-event-publisher' ); ?></option>
										...
										<small><?php esc_html_e( 'برای JetEngine نوع آگهی واقعی را انتخاب کنید؛ اگر چند CPT آگهی دارید گزینه اول را بگذارید.', 'wp-event-publisher' ); ?></small>
									</label>
								<?php endif; ?>
					...
					<article class="jarchi-rule<?php echo $wpep_on ? ' is-on' : ' is-off'; ?>">
						<div class="jarchi-rule__main">
							...
							<?php if ( 'custom_hook' === $wpep_trigger ) : ?>
								<?php $wpep_hook_slug = sanitize_key( (string) ( $wpep_rule['hook_slug'] ?? '' ) ); ?>
								<p class="jarchi-rule__note">
									<?php if ( 'post_bumped' === $wpep_hook_slug ) : ?>
										<?php esc_html_e( 'جارچی شروع نردبان را از تغییرات واقعی Meta آگهی (bump / boost / ladder / nardeban / featured) تشخیص می‌دهد؛ رویداد post_bumped هم به‌عنوان مسیر مستقیم پشتیبانی می‌شود.', 'wp-event-publisher' ); ?>
									<?php elseif ( 'post_bump_expiring' === $wpep_hook_slug ) : ?>
										<?php esc_html_e( 'این قانون از تاریخ واقعی پایان نردبان استفاده می‌کند و جارچی هشدار را برای ۴۸ ساعت قبل از انقضا زمان‌بندی و هر ساعت زمان‌بندی را کنترل می‌کند.', 'wp-event-publisher' ); ?>
									<?php else : ?>
										<?php esc_html_e( 'این رویداد را سایت شما باید اعلام کند:', 'wp-event-publisher' ); ?>
										<code>do_action( 'jarchi_automation_event', '<?php echo esc_html( $wpep_hook_slug ); ?>', $user_id, $context );</code>
									<?php endif; ?>
								</p>
							<?php endif; ?>
						...
						<div class="jarchi-rule__actions">
							<button type="button" class="button button-primary jarchi-rule__edit" data-jarchi-edit-rule="<?php echo esc_attr( (string) ( $wpep_rule['id'] ?? '' ) ); ?>"><?php esc_html_e( 'ویرایش', 'wp-event-publisher' ); ?></button>
	...
	<?php
	$wpep_rule_copy = array();
	foreach ( $rules as $wpep_rule_for_edit ) {
		$wpep_rule_id = sanitize_key( (string) ( $wpep_rule_for_edit['id'] ?? '' ) );
		if ( '' === $wpep_rule_id ) { continue; }
		$wpep_rule_copy[ $wpep_rule_id ] = array(
			'name'    => (string) ( $wpep_rule_for_edit['name'] ?? '' ),
			'subject' => (string) ( $wpep_rule_for_edit['subject'] ?? '' ),
			'body'    => (string) ( $wpep_rule_for_edit['body'] ?? '' ),
		);
	}
	?>
	<div class="jarchi-rule-edit-modal" data-jarchi-rule-edit-modal hidden>
		<button type="button" class="jarchi-rule-edit-modal__backdrop" data-jarchi-edit-close aria-label="<?php esc_attr_e( 'بستن', 'wp-event-publisher' ); ?>"></button>
		<div class="jarchi-rule-edit-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="jarchi-rule-edit-title">
			<div class="jarchi-rule-edit-modal__head">
				<div>
					<h2 id="jarchi-rule-edit-title"><?php esc_html_e( 'ویرایش تیکت خودکار', 'wp-event-publisher' ); ?></h2>
					<p data-jarchi-edit-name></p>
				</div>
				<button type="button" class="button jarchi-rule-edit-modal__close" data-jarchi-edit-close><?php esc_html_e( 'بستن', 'wp-event-publisher' ); ?></button>
			</div>
			<form method="post" action="<?php echo esc_url( $wpep_post_url ); ?>" class="jarchi-rule-edit-modal__form">
				<?php wp_nonce_field( $template_nonce_action ); ?>
				<input type="hidden" name="action" value="wpep_ticket_template_update" />
				<input type="hidden" name="rule_id" value="" data-jarchi-edit-id />
				<label class="wpep-field">
					<span><?php esc_html_e( 'عنوان تیکت', 'wp-event-publisher' ); ?></span>
					<input type="text" name="subject" value="" required data-jarchi-edit-subject />
				</label>
				<label class="wpep-field">
					<span><?php esc_html_e( 'متن تیکت', 'wp-event-publisher' ); ?></span>
					<textarea name="body" rows="9" required data-jarchi-edit-body></textarea>
				</label>
				<details class="jarchi-rule-edit-modal__tokens">
					<summary><?php esc_html_e( 'متغیرهای قابل استفاده', 'wp-event-publisher' ); ?></summary>
					<div>
						<?php foreach ( $tokens as $wpep_token => $wpep_token_label ) : ?>
							<span><code><?php echo esc_html( $wpep_token ); ?></code> <?php echo esc_html( $wpep_token_label ); ?></span>
						<?php endforeach; ?>
					</div>
				</details>
				<div class="jarchi-rule-edit-modal__actions">
					<button type="button" class="button" data-jarchi-edit-close><?php esc_html_e( 'انصراف', 'wp-event-publisher' ); ?></button>
					<button type="submit" class="button button-primary"><?php esc_html_e( 'ذخیره تغییرات', 'wp-event-publisher' ); ?></button>
				</div>
			</form>
		</div>
	</div>
	<style>
	.jarchi-rule-edit-modal[hidden]{display:none!important}.jarchi-rule-edit-modal{position:fixed;inset:0;z-index:100100;display:grid;place-items:center;padding:24px}.jarchi-rule-edit-modal__backdrop{position:absolute;inset:0;border:0;background:rgba(15,23,42,.52);cursor:default}.jarchi-rule-edit-modal__dialog{position:relative;width:min(720px,calc(100vw - 32px));max-height:calc(100vh - 48px);overflow:auto;background:#fff;border-radius:18px;box-shadow:0 24px 80px rgba(15,23,42,.28);padding:22px;box-sizing:border-box}.jarchi-rule-edit-modal__head{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;margin-bottom:18px}.jarchi-rule-edit-modal__head h2{margin:0 0 5px;font-size:20px}.jarchi-rule-edit-modal__head p{margin:0;color:#646970}.jarchi-rule-edit-modal__form{display:grid;gap:15px}.jarchi-rule-edit-modal textarea{width:100%;min-height:190px;line-height:1.9;resize:vertical}.jarchi-rule-edit-modal input[type=text]{width:100%}.jarchi-rule-edit-modal__tokens{border:1px solid #dcdcde;border-radius:12px;padding:11px 13px;background:#f8fafc}.jarchi-rule-edit-modal__tokens summary{cursor:pointer;font-weight:700}.jarchi-rule-edit-modal__tokens>div{display:flex;flex-wrap:wrap;gap:8px 14px;margin-top:10px}.jarchi-rule-edit-modal__tokens span{font-size:12px;color:#50575e}.jarchi-rule-edit-modal__tokens code{direction:ltr}.jarchi-rule-edit-modal__actions{display:flex;justify-content:flex-start;gap:10px;padding-top:4px}@media(max-width:782px){.jarchi-rule-edit-modal{padding:12px}.jarchi-rule-edit-modal__dialog{width:100%;max-height:calc(100vh - 24px);padding:16px}.jarchi-rule-edit-modal__head{align-items:center}}
	</style>
	<script>
	(function(){
		var data=<?php echo wp_json_encode( $wpep_rule_copy, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ); ?>;
		var modal=document.querySelector('[data-jarchi-rule-edit-modal]');
		if(!modal)return;
		var idField=modal.querySelector('[data-jarchi-edit-id]');
		var subject=modal.querySelector('[data-jarchi-edit-subject]');
		var body=modal.querySelector('[data-jarchi-edit-body]');
		var name=modal.querySelector('[data-jarchi-edit-name]');
		var lastFocus=null;
		function openModal(id,button){var rule=data[id];if(!rule)return;lastFocus=button||null;idField.value=id;subject.value=rule.subject||'';body.value=rule.body||'';name.textContent=rule.name||'';modal.hidden=false;document.documentElement.style.overflow='hidden';setTimeout(function(){subject.focus();},0);}
		function closeModal(){modal.hidden=true;document.documentElement.style.overflow='';if(lastFocus&&lastFocus.focus)lastFocus.focus();}
		document.addEventListener('click',function(e){var edit=e.target.closest('[data-jarchi-edit-rule]');if(edit){e.preventDefault();openModal(edit.getAttribute('data-jarchi-edit-rule'),edit);return;}if(e.target.closest('[data-jarchi-edit-close]')){e.preventDefault();closeModal();}});
			...
			<div class="jarchi-rule-form__step">
				...
				<div class="jarchi-rule-form__conditional" data-when-trigger="post_published post_unpublished">
					<label class="wpep-field">
						...
						<select name="post_type">
							<option value="<?php echo esc_attr( WPEventPublisher\TicketAutomations::POST_TYPE_CUSTOM_PUBLIC ); ?>"><?php esc_html_e( 'همه پست‌تایپ‌های سفارشی عمومی (پیشنهادی برای چند نوع آگهی)', 'wp-event-publisher' ); ?></option>
							<?php foreach ( $post_types as $wpep_type ) : ?>
								<option value="<?php echo esc_attr( (string) $wpep_type->name ); ?>"><?php echo esc_html( (string) ( $wpep_type->labels->singular_name ?? $wpep_type->name ) ); ?></option>
							<?php endforeach; ?>
						</select>
						<small><?php esc_html_e( 'یک نوع مشخص را انتخاب کنید، یا برای سایت‌هایی که چند CPT آگهی دارند گزینه «همه پست‌تایپ‌های سفارشی عمومی» را بگذارید.', 'wp-event-publisher' ); ?></small>
					</label>
				</div>
