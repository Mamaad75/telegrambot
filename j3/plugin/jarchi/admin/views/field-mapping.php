...
<div class="wrap wpep-wrap wpep-fields-wrap">
	...
	<p class="description">
		<?php esc_html_e( 'فیلدها بر اساس کاری که می‌کنند دسته‌بندی شده‌اند — عنوان، قیمت، مکان، تماس — نه بر اساس افزونه‌ای که ذخیره‌شان می‌کند. اگر نمی‌دانید از کجا شروع کنید، «پیشنهاد جارچی» یک انتخاب معقول را یک‌جا اعمال می‌کند و بعد هرچه خواستید عوض کنید.', 'wp-event-publisher' ); ?>
	</p>
	...
	<div class="wpep-fields-controls">
		...
		<button type="button" class="button button-secondary" id="wpep-suggest">
			<?php esc_html_e( '✨ پیشنهاد جارچی', 'wp-event-publisher' ); ?>
		</button>
		...
		<article class="wpep-field-card wpep-card" data-key="">
			...
			<div class="wpep-card__body wpep-field-card__body">
				...
				<details class="wpep-field-card__advanced">
					...
					<!-- `visibility` is a legacy compatibility marker. Publication routing
					     is controlled only by the master switch + platform chips above, so
					     administrators cannot accidentally create contradictory states. -->
					<input type="hidden" class="wpep-visibility" value="<?php echo esc_attr( Field::VISIBILITY_BACKEND ); ?>" />
