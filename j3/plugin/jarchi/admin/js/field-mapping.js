...
( function ( $ ) {
	...
	function apply( data ) {
		...
		state.suggested = data.suggested || { keys: [], order: {} };
	...
	function renderTable( order ) {
		...
		/*
		 * Grouped by what a field is for, not by which plugin stores it.
		 *
		 * "JetEngine" and "ACF" answer "where did this come from", which is
		 * not the question anybody opens this screen with. Somebody deciding
		 * what an advert should show wants the price beside the price — and
		 * price and phone used to sit in whichever provider group they
		 * happened to belong to, screens apart.
		 *
		 * The provider is still on each card, so the old question is still
		 * answerable; it is just no longer the organising principle.
		 */
		var sections = state.sections || {};
		var bySection = {};

		Object.keys( sections ).forEach( function ( id ) {
			bySection[ id ] = { id: id, label: sections[ id ], fields: [] };
		} );

		state.groups.forEach( function ( group ) {
			( group.fields || [] ).forEach( function ( field ) {
				var id = field.section && bySection[ field.section ] ? field.section : 'other';

				if ( ! bySection[ id ] ) {
					bySection[ id ] = { id: id, label: id, fields: [] };
				}
		...
		var ordered = Object.keys( bySection ).map( function ( id ) {
			return bySection[ id ];
		} );
	...
	function collect() {
		...
		$( '#wpep-fields-table .wpep-field-card' ).each( function () {
			...
			var enabled = $row.find( '.wpep-enabled' ).is( ':checked' );
			var platformMap = {};
			var anyPlatform = false;

			$row.find( '.wpep-platform' ).each( function () {
				var on = $( this ).is( ':checked' );
				platformMap[ $( this ).attr( 'data-platform' ) ] = on ? 1 : 0;
				anyPlatform = anyPlatform || on;
			} );
			...
			rows.push( {
				...
				// Legacy marker only. The platform map below is authoritative.
				visibility: ! enabled ? 'hidden' : ( anyPlatform ? 'telegram' : 'backend' ),
	...
	$( function () {
		...
		/*
		 * One press to a message that reads like a classified advert: what it
		 * is, what it costs, where it is, how to reach them. A fresh post type
		 * arrives with forty discovered fields and no way to get there by
		 * clicking, so this is a starting point — everything it sets stays
		 * editable, and nothing is saved until Save is pressed.
		 */
		$( '#wpep-suggest' ).on( 'click', function () {
			var suggestion = state.suggested || {};
			var keys = suggestion.keys || [];
			var order = suggestion.order || {};
			...
			$( '#wpep-fields-table .wpep-field-card' ).each( function () {
				var $card = $( this );
				// The card carries its key in data-key, which is also what save reads.
				var key = $card.attr( 'data-key' );
				var wanted = keys.indexOf( String( key ) ) !== -1;

				$card.find( '.wpep-enabled' ).prop( 'checked', wanted );

				if ( wanted && order[ key ] ) {
					$card.find( '.wpep-order' ).val( order[ key ] );
				}

				// Every platform for the fields that are in. A suggestion that
				// switched a field on but left it invisible everywhere would
				// look like it had done nothing.
				$card.find( '.wpep-platform' ).prop( 'checked', wanted );

				reflectCardState( $card );
			} );

			// The builder is left alone: it is the administrator's own layout,
			// and silently replacing it is not a suggestion.
			$( '#wpep-fields-notice' )
				.removeAttr( 'hidden' )
				.removeClass( 'is-error' )
				.text( ( i18n.suggestApplied || '' ).replace( '%d', keys.length ) );
