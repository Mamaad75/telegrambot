...
class Normalizer {
	...
	public function normalize( WP_Post $post, string $event = Event::TYPE_CREATED ): array {
		...
		// Contact is deliberately separate from the advertisement's own phone
		// field. When an administrator enables the contact button, the number
		// comes from the WordPress user who authored the listing — never from a
		// guessed post-meta field. Only platforms whose contact button is enabled
		// receive permission to use it.
		$payload['contact_phone_platforms'] = $this->contact_phone_platforms( $payload['publication_targets'], $author_phone );
		$payload['contact_phone'] = in_array( true, $payload['contact_phone_platforms'], true ) ? $author_phone : '';
	...
	private function buttons( array $targets ): array {
		...
		foreach ( Field::PLATFORMS as $platform ) {
			...
			if ( ! $buttons['contact']['enabled'] && ! empty( $config['contact_button'] ) ) {
				$mode = 'support' === ( $config['contact_mode'] ?? 'author' ) ? 'support' : 'author';
				$url  = 'support' === $mode
					? $this->support_contact_url( (string) ( $config['contact_target'] ?? '' ), $platform )
					: '';

				/*
				 * A support button with nowhere to go is worse than no button:
				 * it looks like a way to reach somebody and is not. If the mode
				 * is 'support' but no destination survived validation, the
				 * button is left off rather than rendered dead.
				 */
				if ( 'support' === $mode && '' === $url ) {
					continue;
				}
				...
				$buttons['contact'] = array(
					...
					// 'author' reveals the advertiser's number and needs one to
					// exist; 'support' is a link and needs nothing from the post,
					// so it shows on every advert.
					'mode'    => $mode,
					'url'     => $url,
				);
			}
	 ...
	 */
	/**
	 * Per-platform permission for the contact button's author phone.
	 *
	 * This does not use the mapped `phone` field. A site's listing phone can be
	 * hidden while the explicit contact button still reaches the account owner,
	 * which is the behaviour the control promises.
	 *
	 * @return array<string,bool> Platform => may use the author's phone.
	 */
	/**
	 * Turns a fixed support account into a link for one platform.
	 *
	 * A username means a different host on each messenger, so it is resolved
	 * here rather than stored per platform — the administrator types the ID
	 * once and it reaches the right place on Telegram and on Bale.
	 *
	 * A full URL that has already been validated is used as it stands, which
	 * is how a site points the button at WhatsApp, a form, or a support page.
	 *
	 * @since 1.26.7
	 *
	 * @param string $target   Username or URL.
	 * @param string $platform Platform key.
	 *
	 * @return string Absolute URL, or '' when there is nothing to link to.
	 */
	private function support_contact_url( string $target, string $platform ): string {
		$target = trim( $target );
		...
		$hosts = array(
			Field::PLATFORM_TELEGRAM => 'https://t.me/',
			Field::PLATFORM_BALE     => 'https://ble.ir/',
		);

		// WhatsApp addresses people by number, not by username, so a bare
		// username has no meaning there. A site wanting a WhatsApp button
		// supplies the wa.me link instead, which the branch above passes on.
		if ( ! isset( $hosts[ $platform ] ) ) {
			return '';
		}

		return $hosts[ $platform ] . rawurlencode( $target );
	}

	private function contact_phone_platforms( array $targets, string $author_phone ): array {
		$site = $this->settings instanceof Settings ? $this->settings->platforms() : array();
		$result = array();
		$has_phone = '' !== trim( $author_phone );

		foreach ( Field::PLATFORMS as $platform ) {
			$config = isset( $site[ $platform ] ) && is_array( $site[ $platform ] ) ? $site[ $platform ] : array();
			$result[ $platform ] = $has_phone
				&& ! empty( $targets[ $platform ]['enabled'] )
				&& ! empty( $config['contact_button'] );
		}
	...
	private function phone_is_publishable( WP_Post $post, array $targets ): bool {
		...
		/*
		 * A fourth gate, and the only one an administrator can close without
		 * also closing the contact button.
		 *
		 * Printing the number in the body and offering a button that reaches
		 * the advertiser were the same decision, so a site that wanted the
		 * button had to publish the number in the text as well. Once a support
		 * desk sits in between that is exactly backwards: the button should
		 * reach the desk and the number should stay off the post.
		 *
		 * Defaults to true, so a site that has never opened this control
		...
		foreach ( $live as $platform ) {
			...
			if ( ! array_key_exists( 'show_phone', $config ) || ! empty( $config['show_phone'] ) ) {
				return true;
			}
		}
