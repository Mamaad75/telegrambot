...
class WooCommerceOrders {
	...
	public function register(): void {
		...
		/*
		 * Creation, and every transition afterwards.
		 *
		 * `woocommerce_checkout_order_created` fires only for orders that come
		 * through the checkout form. An order created in wp-admin, over the
		 * REST API, by a subscription renewal, or by a payment gateway that
		 * builds the order itself never reaches it — so on shops that take
		 * orders any of those ways, "new order" was switched on and silently
		 * never fired. `woocommerce_new_order` covers all of them.
		 *
		 * Both are registered. They overlap on a checkout order, which is
		 * exactly what the per-order idempotency marker in maybe_send() is for:
		 * the second one to arrive finds `order.created` already recorded and
		 * stops. That marker was always load-bearing — WooCommerce can reach
		 * the same status twice on its own — so this adds no new risk.
		 */
	...
	/**
	 * Handles an order created anywhere other than the checkout form.
	 *
	 * `woocommerce_new_order` passes the id first and the object second, which
	 * is the opposite of the checkout hook — hence the separate entry point
	 * rather than one callback with a guessed signature.
	 *
	 * @since 1.26.7
	 *
	 * @param int   $order_id Order identifier.
	 * @param mixed $order    Order object, when WooCommerce supplies one.
	 *
	 * @return void
	 */
	public function on_new_order( $order_id, mixed $order = null ): void {
		$order_id = $order instanceof \WC_Order ? $order->get_id() : absint( $order_id );

		if ( $order_id <= 0 ) {
			return;
		}

		$this->maybe_send( $order_id, 'order.created' );
	}
