...
class FieldRegistry {
	...
	/**
	 * Discovers the catalogue for a concrete post, including rare runtime
	 * meta that a global post-type scan may not have seen.
	 *
	 * @since 1.20.0
	 *
	 * @param WP_Post $post  Concrete post.
	 * @param bool    $fresh Refresh global provider discovery first.
	 * @return array<string,Field> Fields keyed by stable field key.
	 */
	public function discover_for_post( WP_Post $post, bool $fresh = false ): array {
		$fields = $this->discover( $post->post_type, $fresh );
		$meta_provider = $this->provider( MetaProvider::ID );

		if ( ! $meta_provider instanceof MetaProvider ) {
			return $fields;
		}

		foreach ( $meta_provider->discover_post( $post ) as $field ) {
			if ( ! $field instanceof Field || '' === $field->key() ) {
				continue;
			}

			// A framework field already describing this exact storage key is
			// richer than the generic runtime description.
			$already = false;
			foreach ( $fields as $existing ) {
				if ( $existing instanceof Field && $existing->storage_key() === $field->storage_key() ) {
					$already = true;
					break;
				}
			}
			...
			if ( isset( $fields[ $key ] ) && $fields[ $key ]->storage_key() !== $field->storage_key() ) {
				$key = MetaProvider::ID . '_' . $key;
				$field = Field::from_array( array_merge( $field->to_array(), array( 'key' => $key ) ) );
			}

			$fields[ $key ] = $field;
		}

		$fields = (array) apply_filters( 'wpep_discovered_fields_for_post', $fields, $post );

		// Keep concrete runtime discoveries in the request memo. The field
		// mapping screen deliberately samples several recent posts in one
		// request; without this assignment every sample started again from the
		// global catalogue and rare JetEngine/theme meta from the previous post
		// disappeared before the next one was inspected.
		$this->memo[ $post->post_type ] = $fields;
