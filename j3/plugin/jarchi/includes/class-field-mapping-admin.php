...
class FieldMappingAdmin {
	...
	public function enqueue( string $hook_suffix ): void {
		...
		$view = isset( $_GET['jarchi_view'] ) ? sanitize_key( wp_unslash( (string) $_GET['jarchi_view'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$is_central_fields_view = Admin::MENU_SLUG === $page && 'fields' === $view;
		if ( self::PAGE_SLUG !== $page && ! str_contains( $hook_suffix, self::PAGE_SLUG ) && ! $is_central_fields_view ) {
			return;
		}
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'    => array(
					...
					/* translators: %d: how many fields the suggestion switched on. */
					'suggestApplied' => __( 'پیشنهاد جارچی اعمال شد: %d فیلد انتخاب شد. برای ثبت، «ذخیره» را بزنید.', 'wp-event-publisher' ),
	...
	public function render(): void {
		...
		$initial_state = '' !== (string) $current ? $this->build_state( (string) $current, $initial_taxonomy, 0, true ) : array();
	...
	public function ajax_load_fields(): void {
		...
		try {
			...
			error_log( '[Jarchi fields] ' . $e->getMessage() ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			wp_send_json_error(
				array(
					'message' => __( 'خواندن فیلدها با خطا مواجه شد. «بررسی دوباره فیلدها» را بزنید؛ اگر ادامه داشت لاگ PHP را بررسی کنید.', 'wp-event-publisher' ),
				),
				500
			);
		}
	...
	public function ajax_rescan_fields(): void {
		...
		try {
			...
			error_log( '[Jarchi fields rescan] ' . $e->getMessage() ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			wp_send_json_error(
				array( 'message' => __( 'بررسی دوباره فیلدها کامل نشد. خطای دقیق در لاگ PHP ثبت شد.', 'wp-event-publisher' ) ),
				500
			);
		}
	}
	...
	public function ajax_save_mapping(): void {
		...
		$this->prime_runtime_discovery( $post_type, $taxonomy, $term_id, false );
	 ...
	 */
	/**
	 * Primes field discovery with real recent posts from the selected scope.
	 *
	 * Global provider discovery is intentionally bounded and definition-based.
	 * This companion pass is what makes category/form-created fields truly
	 * dynamic: if a key exists on a real recent listing it becomes available in
	 * the mapper and, once enabled, the publication payload.
	 */
	private function prime_runtime_discovery( string $post_type, string $taxonomy, int $term_id, bool $fresh ): void {
		$this->registry->discover( $post_type, $fresh );
		...
		foreach ( $posts as $post ) {
			if ( $post instanceof WP_Post ) {
				$this->registry->discover_for_post( $post );
			}
		}
	}
	...
	private function build_state( string $post_type, string $taxonomy, int $term_id, bool $fresh ): array {
		...
		// Definitions from ACF/JetEngine are only half the story on dynamic
		// classifieds sites. Some fields are created per form/category and only
		// exist as real post meta. Sample recent concrete posts so those fields
		// become selectable in the mapping UI instead of being silently absent.
		$this->prime_runtime_discovery( $post_type, $taxonomy, $term_id, $fresh );

		$grouped = $this->registry->grouped( $post_type, false );
		$mapping = $this->mapping->resolve( $post_type, $taxonomy, $term_id, $sample );
		$scope   = FieldMapping::scope( $post_type, $taxonomy, $term_id );
		...
		foreach ( $grouped as $source => $group ) {
			...
			foreach ( $group['fields'] as $key => $field ) {
				...
				$rows[] = array(
					...
					/*
					 * What the field is for, as opposed to which plugin
					 * supplies it. The screen groups on this; `source` is kept
					 * so a card can still say where it came from.
					 */
					'section'    => FieldSemantics::classify( (string) $key, $field->label(), $field->type(), $field->storage_key() ),
				);
			}
		...
		/*
		 * One flat list of everything discovered, for the suggestion. It is
		 * computed here rather than in the browser so the rule that decides a
		 * sensible starting selection is testable without a browser.
		 */
		$all_rows = array();
		...
		return array(
			...
			'suggested'    => FieldSemantics::suggest( $all_rows ),
