...
class MetaProvider extends BaseProvider {
	...
	/**
	 * Discovers the real meta keys carried by one concrete post.
	 *
	 * Global discovery is intentionally bounded; publication must not be.
	 * This closes the gap for rare/dynamic fields that exist on the post being
	 * sent but fall outside the most-used meta keys for that post type.
	 *
	 * @since 1.20.0
	 *
	 * @param WP_Post $post Post being published.
	 * @return Field[] Fields present on the post.
	 */
	public function discover_post( WP_Post $post ): array {
		$all = get_post_meta( (int) $post->ID );
		if ( ! is_array( $all ) ) {
			return array();
		}
		...
		foreach ( array_keys( $all ) as $storage_key ) {
			$storage_key = (string) $storage_key;

			if ( '' === $storage_key || $this->is_internal( $storage_key ) ) {
				continue;
			}

			$value = $this->meta( (int) $post->ID, $storage_key );
			$key   = Field::sanitize_key( $storage_key );
			...
			$fields[] = new Field(
				array(
					'key'         => $key,
					'label'       => $this->humanize( $storage_key ),
					'storage_key' => $storage_key,
					'source'      => self::ID,
					'type'        => $this->guess_type( $storage_key, $value ),
					'repeatable'  => is_array( maybe_unserialize( $value ) ),
					'meta'        => array( 'runtime' => true ),
				)
			);
		}
