...
class DynamicPayload {
	...
	public function describe( WP_Post $post, array $mapping ): array {
		$discovered = $this->registry->discover_for_post( $post );
		$fields     = array();
		$meta       = array();
		...
		foreach ( $mapping as $key => $settings ) {
			...
			/*
			 * `platforms` is authoritative on current mappings. A field may still
			 * carry a legacy `hidden`/`admin` visibility from an older save while
			 * its Telegram/Bale/WhatsApp switch is explicitly on. Such a field must
			 * leave WordPress so the backend can apply per-platform policy. Legacy
			 * mappings without a platforms map keep the old telegram/backend rule.
			 */
			$transmitted = FieldMapping::goes_anywhere( $settings )
				|| in_array( $visibility, array( Field::VISIBILITY_TELEGRAM, Field::VISIBILITY_BACKEND ), true );
