...
class Validator {
	...
	public function sanitize_platforms( mixed $value ): array {
		...
		foreach ( Field::PLATFORMS as $platform ) {
			...
			$entry = array(
				...
				'contact_mode'   => in_array( $raw['contact_mode'] ?? '', array( 'author', 'support' ), true )
					? (string) $raw['contact_mode']
					: 'author',
				'contact_target' => $this->sanitize_contact_target( $raw['contact_target'] ?? '' ),
				// Defaults to true, so an existing site that has never seen
				// this control keeps printing the number exactly as before.
				'show_phone'     => $this->sanitize_bool( $raw['show_phone'] ?? true ),
			);
	 ...
	 */
	/**
	 * Cleans the fixed support account a contact button may point at.
	 *
	 * Three shapes are accepted, because three are what people have to hand:
	 * a messenger username with or without the `@`, and a full https link for
	 * anything else — a WhatsApp wa.me address, a support page, a form.
	 *
	 * Anything else is refused rather than passed through. The value ends up
	 * in a button URL that the plugin hands to the backend, and a button is a
	 * poor place to discover that a string was not what it looked like.
	 *
	 * @since 1.26.7
	 *
	 * @param mixed $value Raw value.
	 *
	 * @return string Username without the `@`, an https URL, or ''.
	 */
	private function sanitize_contact_target( mixed $value ): string {
		if ( ! is_scalar( $value ) ) {
			return '';
		}
		...
		/*
		 * A messenger link is a username wearing a URL, and it is reduced to
		 * the username first. Kept whole, a pasted t.me address would be sent
		 * to Bale unchanged — the button would be there and would open the
		 * wrong messenger. As a username it is resolved per platform instead.
		 */
		$username = preg_replace( '#^(?:https?://)?(?:www\.)?(?:t\.me|telegram\.me|ble\.ir)/#i', '', $value );
		...
		// Anything else that is a real link is kept as written: wa.me, a
		// support page, a form. Those have no per-platform equivalent to
		// resolve, so they travel as they are.
		if ( preg_match( '#^https?://#i', $value ) ) {
			$url = esc_url_raw( $value );

			return strlen( $url ) > 400 ? '' : $url;
		}
