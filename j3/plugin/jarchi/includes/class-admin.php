	...
	public function render_woocommerce(): void {
		...
		$this->render_view(
			...
			array(
				...
				/*
				 * "Where does this send orders? Nothing shows up even though
				 * it is on." The screen named the platform and stopped there —
				 * not which channel, and not whether anything had ever been
				 * sent. Both are on the screen now, so the question can be
				 * answered from the screen that raised it.
				 */
				'recent'     => wpep()->logger()->query(
					array(
						// The stage every order event is logged under, so this
						// matches the order pipeline rather than any log line
						// that happens to contain the word.
						'stage'    => 'order.queued',
						'per_page' => 8,
					)
				),
				'skipped'    => wpep()->logger()->query(
					array(
						'stage'    => 'order.skipped',
						'per_page' => 5,
					)
