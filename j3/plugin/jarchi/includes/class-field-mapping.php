...
class FieldMapping {
	 ...
	 */
	public function resolve( string $post_type, string $taxonomy = '', int $term_id = 0, ?WP_Post $post = null ): array {
		...
		$discovered = $post instanceof WP_Post
			? $this->registry->discover_for_post( $post )
			: $this->registry->discover( $post_type );
		$effective  = array();
	...
	public function for_post( WP_Post $post ): array {
		...
		return array(
			'mapping'  => $this->resolve( $post->post_type, $taxonomy, $term instanceof WP_Term ? (int) $term->term_id : 0, $post ),
			'taxonomy' => $taxonomy,
			'term'     => $term,
		);
	}
	...
	private function normalize_entry( array $settings ): array {
		$raw_visibility = (string) ( $settings['visibility'] ?? Field::VISIBILITY_BACKEND );
		$raw_visibility = in_array( $raw_visibility, Field::VISIBILITIES, true ) ? $raw_visibility : Field::VISIBILITY_BACKEND;
		$enabled        = ! empty( $settings['enabled'] );
		$platforms      = self::normalize_platforms( $settings, $raw_visibility, $enabled );

		/*
		 * Since 1.6.0 the platform switches are the publication authority. The
		 * legacy visibility select is retained only for backwards compatibility.
		 * Keeping a stale `hidden`/`admin` value while Telegram is switched on
		 * made the UI say a field would publish while DynamicPayload discarded it.
		 * Reconcile the legacy marker whenever this entry is saved.
		 */
		if ( ! $enabled ) {
			$visibility = Field::VISIBILITY_HIDDEN;
		} elseif ( in_array( true, $platforms, true ) ) {
			$visibility = Field::VISIBILITY_TELEGRAM;
		} elseif ( in_array( $raw_visibility, array( Field::VISIBILITY_BACKEND, Field::VISIBILITY_ADMIN, Field::VISIBILITY_HIDDEN ), true ) ) {
			$visibility = $raw_visibility;
		} else {
			$visibility = Field::VISIBILITY_BACKEND;
		}
