...
final class TicketNotifications {
	...
	/**
	 * Marks every stored notification that belongs to one ticket as read.
	 *
	 * Ticket reply state and the notification-centre state used to drift apart:
	 * opening a ticket cleared `_jarchi_ticket_user_unread`, while the separate
	 * `_jarchi_notifications` entry remained unread forever. That stale record
	 * could later repaint a ticket icon with an unrelated number. Keep both
	 * sources consistent at the same read boundary.
	 */
	public function mark_ticket_read( int $user_id, int $ticket_id ): void {
		...
		foreach ( $items as &$item ) {
			$item_ticket = absint( $item['data']['ticket_id'] ?? 0 );
			$type        = sanitize_key( (string) ( $item['type'] ?? '' ) );

			if ( $item_ticket === $ticket_id && in_array( $type, array( 'ticket_reply', 'ticket_created' ), true ) && empty( $item['read'] ) ) {
				$item['read'] = true;
				$changed      = true;
			}
		}
	...
	/**
	 * Repairs legacy notification records whose ticket has already been read.
	 *
	 * This makes the fix self-healing for installations that accumulated stale
	 * ticket notifications before 1.23.7.
	 *
	 * @param array<int,array<string,mixed>> $items Stored notifications.
	 * @return array<int,array<string,mixed>>
	 */
	private function reconcile_ticket_reads( int $user_id, array $items ): array {
		$changed = false;
		...
		foreach ( $items as &$item ) {
			...
			$type = sanitize_key( (string) ( $item['type'] ?? '' ) );
			if ( ! in_array( $type, array( 'ticket_reply', 'ticket_created' ), true ) ) {
				continue;
			}

			$ticket_id = absint( $item['data']['ticket_id'] ?? 0 );
			$ticket    = $ticket_id ? get_post( $ticket_id ) : null;
			if ( ! $ticket || (int) $ticket->post_author !== $user_id ) {
				continue;
			}

			if ( ! get_post_meta( $ticket_id, '_jarchi_ticket_user_unread', true ) ) {
				$item['read'] = true;
				$changed      = true;
			}
		}
	...
	public function rest_summary(): \WP_REST_Response {
		...
		$items         = $this->reconcile_ticket_reads( $user_id, $this->notifications_for( $user_id ) );
		$unread        = array_values( array_filter( $items, static fn( $n ) => empty( $n['read'] ) ) );
		$ticket_unread = wpep()->tickets()->unread_count( $user_id );
