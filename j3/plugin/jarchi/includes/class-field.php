...
class Field {
	...
	public static function sanitize_key( string $raw ): string {
		...
		// A field can legitimately use a non-Latin storage key. Earlier
		// versions reduced a fully Persian key to an empty string and silently
		// dropped it from discovery. Keep existing ASCII identities unchanged,
		// but give otherwise-empty keys a deterministic placeholder-safe id.
		if ( '' === $key && '' !== trim( $raw ) ) {
			$key = 'field_' . substr( md5( $raw ), 0, 12 );
		}
