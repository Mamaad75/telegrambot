...
final class Tickets {
	...
	public const TICKET_URL_OPTION = '_wpep_ticket_front_url';
	...
	public function register(): void {
		...
		/*
		 * Customer ticket forms must never depend on wp-admin/admin-post.php.
		 * Many membership/profile plugins intentionally redirect subscribers away
		 * from every wp-admin URL, including admin-post.php, before WordPress gets
		 * a chance to fire admin_post_*; the result looked exactly like a successful
		 * submit followed by a jump to the account page, but no ticket existed.
		 *
		 * New forms post back to the public ticket page and are handled during
		 * template_redirect. The admin_post hooks stay for old cached markup and
		 * bookmarks, and the priority-0 admin_init bridge catches a normal customer's
		 * legacy post before a typical wp-admin blocker can redirect it.
		 */
		add_action( 'template_redirect', array( $this, 'maybe_disable_ticket_page_cache' ), 0 );
		add_action( 'template_redirect', array( $this, 'handle_frontend_ticket_post' ), 1 );
		add_action( 'admin_init', array( $this, 'handle_legacy_customer_admin_post' ), 0 );
		add_filter( 'rocket_cache_reject_uri', array( $this, 'rocket_reject_ticket_uri' ) );
		add_action( 'init', array( $this, 'upgrade_ticket_front_cache_1238' ), 20 );
		add_action( 'init', array( $this, 'upgrade_ticket_front_cache_1261' ), 21 );
		...
		add_action( 'wp_ajax_wpep_ticket_fresh_nonce', array( $this, 'ajax_fresh_nonce' ) );
	...
	/**
	 * Ticket centres are account-private UI and must never be page-cached.
	 *
	 * Besides stale lists/badges, caching a rendered ticket centre can expose
	 * one customer's ticket titles to another logged-in session. The configured
	 * designer-owned ticket page is therefore explicitly private and Jarchi's
	 * transient navigation URLs are private too.
	 *
	 * @since 1.23.8
	 */
	public function maybe_disable_ticket_page_cache(): void {
		if ( is_admin() || ! $this->is_ticket_front_request() ) {
			return;
		}

		$this->apply_ticket_no_cache();
	}

	/** Apply private/no-store semantics wherever the Ticket Center is rendered. */
	private function apply_ticket_no_cache(): void {
	...
	/** Whether the current front-end request is the configured ticket surface. */
	private function is_ticket_front_request(): bool {
		foreach ( array( 'jarchi_ticket', 'jarchi_view', 'jarchi_form', 'jarchi_ticket_created', 'jarchi_reply_sent', 'jarchi_reply_error', 'ticket_status', 'jarchi_refresh' ) as $key ) {
		...
		if ( $queried_id > 0 && $this->post_contains_ticket_center( $queried_id ) ) {
			return true;
		}

		$configured = trim( (string) get_option( self::TICKET_URL_OPTION, '' ) );
		if ( '' !== $configured && ! empty( $_SERVER['REQUEST_URI'] ) ) {
			$wanted = trailingslashit( (string) wp_parse_url( $configured, PHP_URL_PATH ) );
			$actual = trailingslashit( (string) wp_parse_url( wp_unslash( (string) $_SERVER['REQUEST_URI'] ), PHP_URL_PATH ) );
			if ( '/' !== $wanted && $wanted === $actual ) {
				return true;
			}
		}

		$list_page_id = absint( get_option( '_wpep_ticket_page_id', 0 ) );
		return $list_page_id > 0 && is_page( $list_page_id );
	}

	/** Whether a post/page directly contains Jarchi's ticket center. */
	private function post_contains_ticket_center( int $post_id ): bool {
		if ( $post_id <= 0 ) { return false; }
		$content = (string) get_post_field( 'post_content', $post_id );
		if ( '' !== $content && ( has_shortcode( $content, 'jarchi_tickets' ) || false !== strpos( $content, 'jarchi_ticket_center' ) ) ) {
			return true;
		}
		$elementor = get_post_meta( $post_id, '_elementor_data', true );
		if ( is_string( $elementor ) && ( false !== strpos( $elementor, 'jarchi_ticket_center' ) || false !== strpos( $elementor, '[jarchi_tickets' ) ) ) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Returns paths of published surfaces known to embed Ticket Center.
	 *
	 * WP Rocket can serve a cached response before template_redirect ever runs,
	 * so dynamic detection alone is not enough. The reject list therefore also
	 * includes pages containing the shortcode or the Elementor widget.
	 *
	 * @return string[] URL paths without a leading/trailing slash.
	 */
	private function embedded_ticket_surface_paths(): array {
		global $wpdb;
		$ids = array();
		if ( isset( $wpdb->posts, $wpdb->postmeta ) ) {
			$shortcode_like = '%' . $wpdb->esc_like( '[jarchi_tickets' ) . '%';
			$widget_like    = '%' . $wpdb->esc_like( 'jarchi_ticket_center' ) . '%';
			// A small bounded discovery query is only used while Rocket compiles its
			// exclusion list. It avoids maintaining a second page registry.
			$sql = $wpdb->prepare(
		...
		$list_page_id = absint( get_option( '_wpep_ticket_page_id', 0 ) );
		if ( $list_page_id > 0 ) { $ids[] = $list_page_id; }

		$paths = array();
		foreach ( array_unique( array_filter( $ids ) ) as $id ) {
			$url  = get_permalink( $id );
			$path = is_string( $url ) ? trim( (string) wp_parse_url( $url, PHP_URL_PATH ), '/' ) : '';
			if ( '' !== $path ) { $paths[] = $path; }
		}
	...
	/**
	 * Gives WP Rocket an early path-level exclusion, before cached HTML can be
	 * served and before WordPress has a chance to render the widget.
	 *
	 * @param mixed $uris Existing reject patterns.
	 * @return mixed
	 */
	public function rocket_reject_ticket_uri( $uris ) {
		if ( ! is_array( $uris ) ) {
			return $uris;
		}
		$configured = trim( (string) get_option( self::TICKET_URL_OPTION, '' ) );
		$paths      = $this->embedded_ticket_surface_paths();
		$path       = trim( (string) wp_parse_url( $configured, PHP_URL_PATH ), '/' );
		if ( '' !== $path ) { $paths[] = $path; }
		foreach ( array_unique( array_filter( $paths ) ) as $ticket_path ) {
			$quoted = preg_quote( (string) $ticket_path, '#' );
			$uris[] = '/' . $quoted . '/?';
			$uris[] = '/' . $quoted . '/(.*)';
		}
	...
	/** One-time purge of the old cached ticket page after upgrading. */
	public function upgrade_ticket_front_cache_1238(): void {
		if ( get_option( '_jarchi_ticket_front_cache_1238', false ) ) {
			return;
		}
		$configured = trim( (string) get_option( self::TICKET_URL_OPTION, '' ) );
		if ( '' !== $configured && function_exists( 'rocket_clean_files' ) ) {
			try {
				rocket_clean_files( array( $configured ) );
			} catch ( \Throwable $e ) {
				// Cache purging is optional; the reject rule above is authoritative.
			}
		}
		update_option( '_jarchi_ticket_front_cache_1238', '1', false );
	}


	/**
	 * Purges any already-cached pages that directly embed Ticket Center.
	 *
	 * Runtime no-cache headers prevent regeneration, while this one-time upgrade
	 * removes HTML produced before 1.26.1 taught Rocket about arbitrary widget
	 * locations. Nothing happens when WP Rocket is not active.
	 */
	public function upgrade_ticket_front_cache_1261(): void {
		if ( get_option( '_jarchi_ticket_front_cache_1261', false ) ) { return; }

		if ( function_exists( 'rocket_clean_files' ) ) {
			$urls = array();
			foreach ( $this->embedded_ticket_surface_paths() as $path ) {
			...
			if ( $urls ) {
				try { rocket_clean_files( array_values( array_unique( $urls ) ) ); } catch ( \Throwable $e ) { unset( $e ); }
			}
		}

		update_option( '_jarchi_ticket_front_cache_1261', '1', false );
	}
	...
	/**
	 * Number of tickets that are still open on the support side.
	 *
	 * Every canonical state except `closed` is considered open. Reusing the
	 * grouped status query keeps the sidebar counter cheap even with a large
	 * ticket table and gives the navigation the same numbers as the ticket
	 * dashboard filters.
	 *
	 * @since 1.23.2
	 *
	 ...
	 */
	public function open_ticket_count(): int {
		$counts = $this->ticket_status_counts();

		return max( 0, (int) $counts['all'] - (int) $counts['closed'] );
	}
	...
	public function create_local_automated_ticket( int $customer_id, string $subject, string $body, array $options = array() ): int|\WP_Error {
		...
		$automation_sender = __( 'جارچی', 'wp-event-publisher' );
		foreach ( array( 'department' => self::TAXONOMY, 'category' => self::CATEGORY ) as $key => $taxonomy ) {
			...
			if ( $term_id && term_exists( $term_id, $taxonomy ) ) {
				...
				if ( 'department' === $key ) {
					...
					if ( $term instanceof \WP_Term ) { $automation_sender = sprintf( __( 'جارچی — %s', 'wp-event-publisher' ), (string) $term->name ); }
				}
			}
		}
		update_post_meta( $ticket_id, '_jarchi_ticket_sender_label', $automation_sender );
		$comment_id = wp_insert_comment( array( 'comment_post_ID' => $ticket_id, 'comment_author' => $automation_sender, 'comment_author_email' => '', 'comment_content' => $body, 'comment_type' => self::COMMENT_TYPE, 'user_id' => 0, 'comment_approved' => 1 ) );
		...
		$this->send_backend_ticket_event( 'ticket.created', (int) $ticket_id, array( 'message' => wp_strip_all_tags( $body ), 'source' => 'automation', 'sender_type' => 'system' ) );
	...
	public function register_content_types(): void {
		...
		// 1.23.3: the ticket centre is a true embeddable Elementor/shortcode component.
		// Jarchi no longer creates or requires a dedicated WordPress page. Existing
		// legacy pages are left untouched so upgrades are non-destructive.
	}

	/**
	 * Legacy compatibility hook.
	 *
	 * Since 1.23.3 Jarchi never creates a customer-facing ticket page. The
	 * centre is rendered exactly where the designer places the Elementor widget
	 * (or legacy shortcode). Existing pages/options are intentionally preserved
	 * because deleting a page during an update would be destructive.
	 */
	...
	/**
	 * Converts an untouched Jarchi shortcode/blank page into an Elementor page.
	 * Existing hand-built Elementor layouts are never overwritten.
	 */
	private function maybe_convert_ticket_page_to_elementor( int $page_id, string $view ): void {
		if ( $page_id <= 0 || ! defined( 'ELEMENTOR_VERSION' ) ) {
			return;
		}

		$existing_data = get_post_meta( $page_id, '_elementor_data', true );
		if ( is_string( $existing_data ) && '' !== trim( $existing_data ) && '[]' !== trim( $existing_data ) ) {
			// Existing Elementor pages from older Jarchi releases often contain
			// Elementor's generic Shortcode widget. Upgrade only that exact
			// Jarchi shortcode in-place so the site owner's surrounding layout,
			// spacing and responsive settings stay untouched while the ticket UI
			// becomes a first-class Elementor widget.
			$data = json_decode( wp_unslash( $existing_data ), true );
			if ( is_array( $data ) ) {
				$changed = false;
				$this->upgrade_elementor_ticket_shortcodes( $data, $view, $changed );
				if ( $changed ) {
		...
		if ( '' !== $content && ! preg_match( '/^\[jarchi_tickets(?:\s+view=["\'](?:list|new)["\'])?\]$/', $content ) ) {
			return;
		}

		$widget_id  = substr( md5( 'jarchi-ticket-widget-' . $page_id . '-' . $view ), 0, 8 );
		$column_id  = substr( md5( 'jarchi-ticket-column-' . $page_id . '-' . $view ), 0, 8 );
		$section_id = substr( md5( 'jarchi-ticket-section-' . $page_id . '-' . $view ), 0, 8 );
		$data = array(
			array(
				'id' => $section_id, 'elType' => 'section', 'isInner' => false, 'settings' => array(),
				'elements' => array(
					array(
						'id' => $column_id, 'elType' => 'column', 'isInner' => false, 'settings' => array( '_column_size' => 100, '_inline_size' => null ),
						'elements' => array(
							array(
								'id' => $widget_id, 'elType' => 'widget', 'widgetType' => 'jarchi_ticket_center',
								'settings' => array( 'view' => $view ), 'elements' => array(),
							),
						),
					),
				),
	...
	/**
	 * Recursively replaces legacy Elementor Shortcode widgets that render the
	 * Jarchi ticket shortcode with the native Jarchi ticket widget.
	 *
	 * @param array<int|string,mixed> $elements Elementor document/elements.
	 * @param string                  $fallback_view View for a shortcode without an explicit view.
	 * @param bool                    $changed Set true when at least one widget is upgraded.
	 */
	private function upgrade_elementor_ticket_shortcodes( array &$elements, string $fallback_view, bool &$changed ): void {
		foreach ( $elements as &$element ) {
			...
			if (
				...
				$shortcode = trim( (string) $element['settings']['shortcode'] );
				if ( preg_match( '/^\[jarchi_tickets(?:\s+view=["\'](list|new)["\'])?\]$/', $shortcode, $match ) ) {
					$settings = is_array( $element['settings'] ) ? $element['settings'] : array();
					unset( $settings['shortcode'] );
					$settings['view'] = isset( $match[1] ) && in_array( $match[1], array( 'list', 'new' ), true ) ? $match[1] : $fallback_view;
					$element['widgetType'] = 'jarchi_ticket_center';
			...
			if ( ! empty( $element['elements'] ) && is_array( $element['elements'] ) ) {
				$this->upgrade_elementor_ticket_shortcodes( $element['elements'], $fallback_view, $changed );
			}
		}
		unset( $element );
	}
	...
	public function ticket_page_url(): string {
		/*
		 * 1.23.4: Jarchi no longer owns a front-end ticket page. The canonical
		 * destination is explicitly chosen by the site designer/admin. This keeps
		 * notification links, shortcodes and menu badges aligned with the real
		 * Elementor/JetEngine page and prevents redirects to the removed legacy
		 * /jarchi-tickets/ route.
		 */
		$configured = trim( (string) get_option( self::TICKET_URL_OPTION, '' ) );
		if ( '' !== $configured ) {
			$validated = wp_validate_redirect( esc_url_raw( $configured ), '' );
			$home_host = strtolower( (string) wp_parse_url( home_url( '/' ), PHP_URL_HOST ) );
			$url_host  = strtolower( (string) wp_parse_url( $validated, PHP_URL_HOST ) );
			if ( '' !== $validated && ( '' === $url_host || $home_host === $url_host ) ) {
				return (string) apply_filters( 'wpep_ticket_page_url', $validated );
			}
		}

		// Legacy installs may still have an intentionally kept page id. It remains
		// a valid fallback, but Jarchi never creates it anymore.
		$page_id = absint( get_option( '_wpep_ticket_page_id', 0 ) );
		if ( $page_id && 'publish' === get_post_status( $page_id ) ) {
			return (string) apply_filters( 'wpep_ticket_page_url', get_permalink( $page_id ) );
		}

		// Never invent /jarchi-tickets/. A missing configuration falls back to the
		// site home instead of a guaranteed 404; embedded ticket centres still use
		// current_embed_url() and therefore remain on the designer-owned page.
		return (string) apply_filters( 'wpep_ticket_page_url', home_url( '/' ) );
	}

	public function new_ticket_page_url(): string {
		// Creation now lives inside the same ticket-centre page. Keep the query
		// flag only as a no-JS / legacy deep-link that chooses the initial state.
		return add_query_arg( 'jarchi_view', 'new', $this->ticket_page_url() );
	}

	/**
	 * URL of the front-end page that is currently hosting an embedded centre.
	 *
	 * JetEngine/Elementor layouts can place the widget inside any account page,
	 * including URLs with their own query arguments. Preserve those arguments and
	 * remove only Jarchi's transient navigation flags so every ticket action comes
	 * back to the exact designer-owned page instead of /jarchi-tickets/.
	 */
	public function current_embed_url(): string {
		$candidate = '';
		...
		if ( '' === $candidate && get_queried_object_id() ) {
		...
		return $this->clean_embed_url( $candidate );
	}

	/**
	 * Removes only ticket-state arguments from a host page URL.
	 */
	private function clean_embed_url( string $url ): string {
		$url = esc_url_raw( $url );
		if ( '' === $url ) {
			return $this->ticket_page_url();
		}
	...
	/**
	 * Validated return destination supplied by an embedded ticket form.
	 *
	 * The form nonce protects the action; this additionally constrains the return
	 * URL to this WordPress site's host so it cannot become an open redirect.
	 */
	private function posted_frontend_return_url(): string {
		$raw = isset( $_POST['jarchi_return_url'] )
			? esc_url_raw( wp_unslash( (string) $_POST['jarchi_return_url'] ) )
			: '';
		...
		if ( '' !== $raw ) {
			...
			$url_host  = strtolower( (string) wp_parse_url( $validated, PHP_URL_HOST ) );
			if ( '' !== $validated && ( '' === $url_host || $home_host === $url_host ) ) {
				return $this->clean_embed_url( $validated );
			}
		}
	...
	/**
	 * Human-readable sender/receiver for the admin inbox.
	 *
	 * A ticket post always belongs to the customer, even when the site or an
	 * automation started the conversation. Therefore post_author alone cannot
	 * describe direction. New tickets store an origin marker; legacy tickets
	 * fall back to the first message sender so the table remains truthful.
	 *
	 * @since 1.23.8
	 * @return array{sender:string,receiver:string,direction:string}
	 */
	public function admin_ticket_parties( int $ticket_id ): array {
		$customer       = get_user_by( 'id', (int) get_post_field( 'post_author', $ticket_id ) );
		$customer_label = $customer ? (string) $customer->display_name : __( 'کاربر حذف شده', 'wp-event-publisher' );
		$department_terms = wp_get_post_terms( $ticket_id, self::TAXONOMY );
		$department = ! is_wp_error( $department_terms ) && ! empty( $department_terms ) ? (string) $department_terms[0]->name : '';
		$agent_id   = absint( get_post_meta( $ticket_id, '_jarchi_ticket_agent', true ) );
		$agent      = $agent_id ? get_user_by( 'id', $agent_id ) : false;
		...
		if ( '' === $origin ) {
			...
			$first_id        = ! empty( $first ) ? (int) $first[0]->comment_ID : 0;
			$first_sender    = $first_id ? (string) get_comment_meta( $first_id, '_jarchi_ticket_sender', true ) : '';
			$first_automated = $first_id ? (string) get_comment_meta( $first_id, '_jarchi_ticket_automated', true ) : '';
			$stored_sender   = trim( (string) get_post_meta( $ticket_id, '_jarchi_ticket_sender_label', true ) );
			$automation_id   = sanitize_key( (string) get_post_meta( $ticket_id, '_jarchi_ticket_automation_id', true ) );

			if ( get_post_meta( $ticket_id, self::META_AUTOMATED, true ) || '1' === $first_automated || '' !== $automation_id || str_starts_with( $stored_sender, 'جارچی' ) ) {
				$origin = 'automation';
			} else {
				$origin = 'admin' === $first_sender ? 'admin' : 'user';
			}
		}

		if ( in_array( $origin, array( 'automation', 'admin', 'remote' ), true ) ) {
			$sender = trim( (string) get_post_meta( $ticket_id, '_jarchi_ticket_sender_label', true ) );
			if ( 'automation' === $origin ) {
				$sender = $department
					? sprintf( __( 'جارچی — %s', 'wp-event-publisher' ), $department )
					: __( 'جارچی', 'wp-event-publisher' );
			} elseif ( '' === $sender ) {
				$sender = $department ?: __( 'مدیریت سایت', 'wp-event-publisher' );
			}
			return array( 'sender' => $sender, 'receiver' => $customer_label, 'direction' => 'outbound' );
		}

		$receiver = $agent ? (string) $agent->display_name : ( $department ?: __( 'پشتیبانی', 'wp-event-publisher' ) );
		return array( 'sender' => $customer_label, 'receiver' => $receiver, 'direction' => 'inbound' );
	}
	...
	public function render_admin(): void {
		...
		if ( '' !== trim( $search ) ) {
			$search_ids = $this->admin_ticket_search_ids( $search );
			$args['post__in'] = $search_ids ?: array( 0 );
		}
	...
	/**
	 * Full support-desk search.
	 *
	 * WordPress' native `s` query searches only the ticket post title/content,
	 * while Jarchi stores the actual conversation in comments and the customer
	 * identity on the ticket author. Support staff reasonably type `#14910`, a
	 * phone number, an email or a sentence from a reply and expect the thread.
	 * Resolve those inputs to ticket ids first, then let WP_Query apply the
	 * existing status/department/category permission constraints.
	 *
	 * @since 1.23.4
	 * @return int[]
	 */
	private function admin_ticket_search_ids( string $term ): array {
		global $wpdb;
		...
		$id_candidate = ltrim( trim( $latin ), "#№ \t\n\r\0\x0B" );
		$needle       = ctype_digit( $id_candidate ) ? $id_candidate : $term;
		$like         = '%' . $wpdb->esc_like( $needle ) . '%';

		$phone_keys = array( 'phone', 'mobile', 'billing_phone', '_billing_phone', 'user_phone', 'user_mobile', 'mobile_number', 'phone_number' );
		$phone_placeholders = implode( ',', array_fill( 0, count( $phone_keys ), '%s' ) );

		$sql = "SELECT DISTINCT p.ID
			FROM {$wpdb->posts} p
			LEFT JOIN {$wpdb->comments} c
				ON c.comment_post_ID = p.ID AND c.comment_type = %s
			LEFT JOIN {$wpdb->users} u ON u.ID = p.post_author
			LEFT JOIN {$wpdb->usermeta} um
				ON um.user_id = u.ID AND um.meta_key IN ({$phone_placeholders})
			WHERE p.post_type = %s
			  AND p.post_status IN ('publish','private','draft','pending')
			  AND (
				p.post_title LIKE %s
				OR p.post_content LIKE %s
				OR c.comment_content LIKE %s
		...
		$params = array_merge(
			array( self::COMMENT_TYPE ),
			$phone_keys,
			array( self::POST_TYPE, $like, $like, $like, $like, $like, $like, $like )
		);

		if ( ctype_digit( $id_candidate ) ) {
			$sql .= ' OR p.ID = %d';
			$params[] = absint( $id_candidate );
		}

		$sql .= ') ORDER BY p.post_modified_gmt DESC LIMIT 500';

		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- placeholders and values are prepared below.
	...
	public function render_ui_settings(): void {
		...
		$hub_notice = '';
		if ( isset( $_POST['jarchi_customer_hub_create'] ) ) {
			check_admin_referer( 'jarchi_customer_hub_page' );
			$mode = sanitize_key( wp_unslash( (string) ( $_POST['hub_mode'] ?? 'both' ) ) );
			$result = wpep()->customer_hub()->create_page( $mode, ! empty( $_POST['hub_stats'] ) );
			$hub_notice = is_wp_error( $result )
				? $result->get_error_message()
				: __( 'صفحه مرکز کاربری ساخته شد. هر زمان خواستید می‌توانید محتوایش را با شورت‌کد یا Elementor تغییر دهید.', 'wp-event-publisher' );
		}
		if ( isset( $_POST['jarchi_customer_hub_remove'] ) ) {
			check_admin_referer( 'jarchi_customer_hub_page' );
			$hub_id = absint( get_option( CustomerHub::OPTION_PAGE, 0 ) );
			if ( $hub_id && 'page' === get_post_type( $hub_id ) ) {
				wp_trash_post( $hub_id );
			}
			delete_option( CustomerHub::OPTION_PAGE );
			$hub_notice = __( 'صفحه مرکز کاربری جارچی حذف شد. تیکت‌ها و اعلان‌ها همچنان با شورت‌کدها و Elementor قابل استفاده‌اند.', 'wp-event-publisher' );
		}
		if ( isset( $_POST['jarchi_ticket_ui_save'] ) ) {
			...
			$settings['show_category'] = ! empty( $_POST['show_category'] );

			$ticket_url = isset( $_POST['ticket_url'] ) ? esc_url_raw( wp_unslash( (string) $_POST['ticket_url'] ) ) : '';
			if ( '' !== $ticket_url ) {
				$validated = wp_validate_redirect( $ticket_url, '' );
				$home_host = strtolower( (string) wp_parse_url( home_url( '/' ), PHP_URL_HOST ) );
				$url_host  = strtolower( (string) wp_parse_url( $validated, PHP_URL_HOST ) );
				if ( '' !== $validated && ( '' === $url_host || $home_host === $url_host ) ) {
					update_option( self::TICKET_URL_OPTION, $validated, false );
				}
			} else {
				delete_option( self::TICKET_URL_OPTION );
			}
	...
	public function render_advanced_settings(): void {
		...
		$bot_settings = $this->ticket_bot_settings();
		if ( isset($_POST['jarchi_ticket_advanced_save']) ) {
			...
			// FAQ is intentionally managed only from the dedicated FAQ screen.
			// Do not overwrite it when saving unrelated advanced settings.
			...
			foreach ( $keys as $i => $key ) {
				...
				$options = sanitize_textarea_field( wp_unslash( $cfopts[ $i ] ?? '' ) );
				$cf[] = array(
					'key'      => $key,
					'label'    => $label,
			...
			$bot_platforms = array_map( 'sanitize_key', (array) ( $_POST['ticket_bot_platforms'] ?? array() ) );
			$bot_platforms = array_values( array_intersect( array( 'telegram', 'bale' ), $bot_platforms ) );
			$bot_settings = array(
				'enabled'   => ! empty( $_POST['ticket_bot_enabled'] ),
				'platforms' => $bot_platforms,
			);
			update_option( '_jarchi_ticket_bot_settings', $bot_settings, false );
		}
	...
	public function canned_replies(): array {
		...
		$items = is_array( $value ) ? array_values( array_filter( $value, 'is_array' ) ) : array();

		foreach ( $items as &$item ) {
			$item['title'] = $this->repair_utf8_mojibake( (string) ( $item['title'] ?? '' ) );
			$item['body']  = $this->repair_utf8_mojibake( (string) ( $item['body'] ?? '' ) );
		}
		unset( $item );
	...
	/**
	 * Repairs the classic UTF-8-as-Latin1 mojibake visible as Ø/Ù/Ã/Â.
	 *
	 * Existing clean Persian text is returned byte-for-byte unchanged.
	 */
	private function repair_utf8_mojibake( string $text ): string {
		if ( '' === $text || ! preg_match( '/[ØÙÃÂ]/u', $text ) || ! function_exists( 'mb_convert_encoding' ) ) {
			return $text;
		}
		...
		$before = preg_match_all( '/[ØÙÃÂ]/u', $text );
		$after  = preg_match_all( '/[ØÙÃÂ]/u', $candidate );

		return false !== $before && false !== $after && $after < $before ? $candidate : $text;
	}
	...
	public function render_bot_settings(): void {
		if ( ! current_user_can( Admin::CAPABILITY ) ) { wp_die( esc_html__( 'اجازه دسترسی ندارید.', 'wp-event-publisher' ) ); }
		// 1.23.0: keep the legacy route/bookmark alive, but settings now live in «پیشرفته».
		wp_safe_redirect( Admin::app_url( 'ticket-advanced', array( 'section' => 'remote-ticketing' ) ) );
		exit;
	}
	...
	public function handle_bot_settings(): void {
		...
		wp_safe_redirect( Admin::app_url( 'ticket-advanced', array( 'section' => 'remote-ticketing', 'updated' => '1' ) ) );
		exit;
	}
		...
		if ( isset( $_POST['jarchi_canned_save'] ) ) {
			...
			if ( '' !== trim( $title ) && '' !== trim( wp_strip_all_tags( $body ) ) ) {
				...
				if ( ! empty( $_POST['add_to_faq'] ) ) {
					$settings = $this->advanced_settings();
					$settings['faq'] = array_values( array_filter( (array) ( $settings['faq'] ?? array() ), 'is_array' ) );
					$settings['faq'][] = array( 'title' => $title, 'body' => $body );
					update_option( '_jarchi_ticket_advanced_settings', $settings, false );
				}
			}
		}
	...
	public function attachment_policy(): array {
		...
		// way to raise either Jarchi's hard cap or PHP's actual upload ceiling.
		$max_files   = min( self::MAX_ATTACHMENTS, max( 1, (int) $advanced['max_files'] ) );
		$max_mb      = min( 64, max( 1, (int) $advanced['max_file_mb'] ) );
		$configured  = $max_mb * MB_IN_BYTES;
		$server_max  = function_exists( 'wp_max_upload_size' ) ? (int) wp_max_upload_size() : $configured;
		$max_bytes   = $server_max > 0 ? min( $configured, $server_max ) : $configured;
		$post_max    = function_exists( 'wp_convert_hr_to_bytes' ) ? (int) wp_convert_hr_to_bytes( (string) ini_get( 'post_max_size' ) ) : 0;
		$max_total   = $post_max > 0 ? max( MB_IN_BYTES, (int) floor( $post_max * 0.90 ) ) : $max_bytes * $max_files;
	...
	public function create_admin_ticket( int $customer_id, array $payload ): int|\WP_Error {
		...
		update_post_meta( $ticket_id, '_jarchi_ticket_origin', 'admin' );
		update_post_meta( $ticket_id, '_jarchi_ticket_sender_label', (string) $author->display_name );
	...
	public function handle_assign_agent(): void {
		...
		if ( ! $this->can_manage_ticket( $ticket_id ) ) {
			wp_die( esc_html__( 'تیکت پیدا نشد.', 'wp-event-publisher' ) );
		}

		$default_return = Admin::app_url( 'support', array( 'ticket' => $ticket_id ) );
		$posted_return  = esc_url_raw( wp_unslash( (string) ( $_POST['return_url'] ?? '' ) ) );
		$return_url     = wp_validate_redirect( $posted_return, $default_return );

		$agent = absint( $_POST['agent'] ?? 0 );
		if ( $agent ) {
			$user = get_user_by( 'id', $agent );
			if ( ! $user || ! in_array( 'jarchi_support_agent', (array) $user->roles, true ) ) {
				wp_safe_redirect( add_query_arg( 'jarchi_agent_error', 'invalid_agent', $return_url ) );
				exit;
			}
		}

		$this->assign_agent( $ticket_id, $agent );

		wp_safe_redirect( add_query_arg( 'jarchi_agent_updated', '1', $return_url ) );
		exit;
	}

	/**
	 * Verifies the public ticket nonce without ever showing WordPress' generic
	 * "link expired" screen to a customer. Cached markup can carry an old nonce;
	 * in that case return to the same embedded Ticket Center with a recoverable
	 * error flag instead of leaving the account layout.
	 */
	private function verify_frontend_nonce_or_redirect( string $return_url, int $ticket_id = 0 ): void {
		...
		$args = array( 'jarchi_refresh' => time() );
		if ( $ticket_id > 0 ) {
			$args['jarchi_reply_error'] = 'expired';
			$args['jarchi_ticket'] = $ticket_id;
		} else {
			$args['jarchi_ticket_error'] = 'expired';
			$args['jarchi_view'] = 'new';
			$args['jarchi_form'] = '1';
		}
	...
	public function save_rating(): void {
		...
		$ticket_id = absint( $_POST['ticket_id'] ?? 0 );
		$this->verify_frontend_nonce_or_redirect( $this->posted_frontend_return_url(), $ticket_id ); $rating = absint( $_POST['rating'] ?? 0 ); $ticket = get_post( $ticket_id );
		...
		wp_safe_redirect( $this->posted_frontend_return_url() ); exit;
	}
	...
	/**
	 * Handles customer-facing ticket forms on the public page.
	 *
	 * @since 1.23.1
	 */
	public function handle_frontend_ticket_post(): void {
		if ( is_admin() || 'POST' !== strtoupper( (string) ( $_SERVER['REQUEST_METHOD'] ?? '' ) ) ) {
			return;
		}

		$front_action = isset( $_POST['jarchi_front_action'] )
			? sanitize_key( wp_unslash( (string) $_POST['jarchi_front_action'] ) )
			: '';

		if ( ! in_array( $front_action, array( 'submit', 'reply', 'rating' ), true ) ) {
			return;
		}

		if ( ! is_user_logged_in() ) {
			wp_safe_redirect( wp_login_url( $this->posted_frontend_return_url() ) );
			exit;
		}
	...
	/**
	 * Compatibility bridge for ticket forms cached from <= 1.23.0.
	 *
	 * Old customer forms posted to wp-admin/admin-post.php. A subscriber-facing
	 * profile plugin can redirect that request to its account page on admin_init,
	 * so process only the ordinary customer's three ticket actions first. Staff
	 * and administrators keep the normal admin_post routing used by the back office.
	 *
	 * @since 1.23.1
	 */
	public function handle_legacy_customer_admin_post(): void {
		if ( 'POST' !== strtoupper( (string) ( $_SERVER['REQUEST_METHOD'] ?? '' ) ) || ! is_user_logged_in() ) {
			return;
		}
		...
		$legacy_action = isset( $_POST['action'] )
			? sanitize_key( wp_unslash( (string) $_POST['action'] ) )
			: '';

		switch ( $legacy_action ) {
			case 'wpep_ticket_submit':
				$this->handle_submit();
				break;
			case 'wpep_ticket_reply':
				$this->handle_reply();
				break;
			case 'wpep_ticket_rating':
				$this->save_rating();
				break;
		}
	}
	...
	public function handle_submit(): void {
		...
		$return_url = $this->posted_frontend_return_url();
		$this->verify_frontend_nonce_or_redirect( $return_url );
		...
		update_post_meta( $ticket_id, '_jarchi_ticket_priority', $priority );
		update_post_meta( $ticket_id, '_jarchi_ticket_admin_unread', '1' );
		update_post_meta( $ticket_id, '_jarchi_ticket_origin', 'user' );
		...
		foreach ( (array) $settings['custom_fields'] as $field ) {
			$key = sanitize_key( $field['key'] ?? '' );
			if ( ! $key ) { continue; }
			$value = wp_unslash( $_POST[ 'custom_field_' . $key ] ?? '' );
				...
				wp_safe_redirect(
					add_query_arg(
						array(
							'jarchi_view'         => 'new',
							'jarchi_form'         => '1',
							'jarchi_ticket_error' => 'required_field',
						),
						$return_url
					)
				);
		...
		/*
		 * After creation the customer returns to the inbox, not the newly-opened
		 * thread. A short-lived query flag lets the same single-page ticket centre
		 * confirm the action without introducing a fourth screen. The ticket id is
		 * included only so the notice can be validated against the current user.
		 */
		$redirect = add_query_arg(
			array(
				'jarchi_ticket_created' => (int) $ticket_id,
				'jarchi_refresh'        => time(),
			),
			$return_url
		);
	...
	public function handle_reply(): void {
		...
		if ( current_user_can( Admin::CAPABILITY ) || current_user_can( 'jarchi_support_tickets' ) ) {
			...
			$this->verify_frontend_nonce_or_redirect( $return_url, absint( $_POST['ticket_id'] ?? 0 ) );
		}
		...
		if ( '' === trim( wp_strip_all_tags( $body ) ) && empty( $_FILES['attachments']['name'][0] ?? '' ) ) {
			wp_safe_redirect( add_query_arg( 'jarchi_ticket', $ticket_id, $return_url ) );
			exit;
		}

		$reply_id = $this->insert_message( $ticket_id, $body, get_current_user_id(), $is_admin ? 'admin' : 'user', $is_admin ? 'answered' : 'waiting' );
		if ( ! $reply_id ) {
			wp_safe_redirect(
				add_query_arg(
					array(
						'jarchi_ticket'      => $ticket_id,
						'jarchi_reply_error' => 'not_saved',
					),
					$return_url
				)
			);
		...
		/*
		 * A support-side reply is an admin workflow. Sending the administrator
		 * to the public customer ticket centre after every answer breaks that
		 * workflow and also hides the freshly-updated status from the screen they
		 * were using. Keep staff on the same WordPress-admin ticket and let the
		 * normal GET render act as the refresh. Customer replies still return to
		 * the public conversation exactly as before.
		 */
		if ( $is_admin ) {
			$redirect = Admin::app_url( 'support', array( 'ticket' => $ticket_id ) );
		} else {
			$redirect = add_query_arg(
				array(
					'jarchi_ticket'     => $ticket_id,
	...
	private function notify_user_now( int $ticket_id, string $body ): void {
		 ...
		 */
		$automated    = $this->is_automated( $ticket_id );
		$started_here = $automated || 1 === count( $this->messages( $ticket_id ) );
		$answerable   = $this->replies_allowed( $ticket_id );

		if ( $automated ) {
			$subject = $ticket->post_title;
			$message = trim( wp_strip_all_tags( $body ) ) . "\n\n" . __( 'مشاهده تیکت:', 'wp-event-publisher' ) . "\n" . $link;
	...
	public function mark_read_for_user( int $ticket_id ): void {
		...
		if ( $owner > 0 ) {
			...
			// A ticket reply also creates a notification-centre record. Treat the
			// act of opening the ticket as one read action everywhere, otherwise a
			// stale notification can keep a badge alive after the ticket was read.
			if ( function_exists( 'wpep' ) ) {
				try {
					$notifications = wpep()->ticket_notifications();
					if ( is_object( $notifications ) && method_exists( $notifications, 'mark_ticket_read' ) ) {
						$notifications->mark_ticket_read( $owner, $ticket_id );
					}
				} catch ( \Throwable $e ) {
					// Reading a ticket must never fail because an optional notification
					// channel is unavailable. The ticket meta remains authoritative.
				}
			}
		}
	}
	...
	public function unread_count( ?int $user_id = null ): int {
		...
		set_transient( $this->unread_cache_key( $user_id ), $count, 5 );
	...
	/**
	 * Returns a just-in-time form nonce to a logged-in browser.
	 *
	 * Ticket pages can sit in a browser tab or an Elementor/WP Rocket cache long
	 * enough for the HTML nonce to expire. Fetching it immediately before POST
	 * prevents WordPress's opaque "link expired" screen without weakening CSRF:
	 * another origin cannot read this same-origin, cookie-authenticated response.
	 *
	 * @since 1.23.8
	 */
	...
	public function filter_ticket_page_content( string $content ): string {
		...
		$list_page_id = absint( get_option( '_wpep_ticket_page_id', 0 ) );
		$new_page_id  = absint( get_option( '_wpep_ticket_new_page_id', 0 ) );
		$current_id   = get_queried_object_id();

		if ( ! $current_id || ( $current_id !== $list_page_id && $current_id !== $new_page_id ) ) {
			return $content;
		}
		...
		// A shortcode (or Elementor widget) owns its own output.
		if ( has_shortcode( $content, 'jarchi_tickets' ) ) {
			return $content;
		}

		$trimmed = trim( wp_strip_all_tags( $content ) );
		if ( '' === $trimmed ) {
			return $this->render_center( array( 'view' => $current_id === $new_page_id ? 'new' : 'list' ) );
		}

		return $content;
	}

	public function shortcode_center( array $atts = array() ): string {
		$atts = shortcode_atts(
			array(
				'view'          => 'list',
				'show_category' => '',
		...
		if ( '' !== (string) $atts['show_category'] ) {
			$atts['show_category'] = in_array( strtolower( (string) $atts['show_category'] ), array( '1', 'yes', 'true', 'on', 'show' ), true );
		} else {
			unset( $atts['show_category'] );
		}
		$atts['category_label'] = sanitize_text_field( (string) $atts['category_label'] );

		$atts['embedded'] = true;
		$atts['base_url'] = $this->current_embed_url();

		return $this->render_center( $atts );
	}

	/**
	 * Canonical front-end renderer.
	 *
	 * 1.20.0 separates the list from creation. The main screen is now a fast
	 * ticket inbox; the new-ticket screen leads with FAQ and reveals the form
	 * only when the customer says the answer did not help.
	 */
	public function render_center( array $overrides = array() ): string {
		$this->apply_ticket_no_cache();
		$embedded      = ! empty( $overrides['embedded'] );
		$embedded_class = $embedded ? ' jarchi-ticket-center--embedded' : '';
		...
		if ( ! is_user_logged_in() ) {
			...
			return '<div class="jarchi-ticket-center' . esc_attr( $embedded_class ) . '"><div class="jarchi-ticket-empty"><strong>' . esc_html( $login_title ) . '</strong><p>' . esc_html( $login_desc ) . '</p></div></div>';
		}

		$this->enqueue_front_assets();

		$base_url = isset( $overrides['base_url'] ) && is_string( $overrides['base_url'] ) && '' !== $overrides['base_url']
			? $this->clean_embed_url( $overrides['base_url'] )
			: $this->ticket_page_url();
		...
		$list_width = isset( $overrides['list_width']['size'] ) ? (float) $overrides['list_width']['size'] . ( $overrides['list_width']['unit'] ?? 'px' ) : '320px';
		...
		$ui_style = sprintf(
			'--jt-primary:%1$s;--jt-primary-hi:%2$s;--jt-surface:%3$s;--jt-bg:%4$s;--jt-text:%5$s;--jt-muted:%6$s;--jt-border:%7$s;--jt-field-bg:%8$s;--jt-field-text:%9$s;--jt-button-text:%10$s;--jt-badge-bg:%11$s;--jt-badge-text:%12$s;--jt-radius:%13$s;--jt-shadow:%14$s;--jt-list-width:%15$s;max-width:%16$s;',
			esc_attr( $ui['primary'] ),
			esc_attr( $ui['primary_hi'] ?? '#FF8A1C' ),
			esc_attr( $ui['surface'] ),
			esc_attr( $ui['background'] ),
			esc_attr( $ui['text'] ),
			esc_attr( $ui['muted'] ),
			esc_attr( $ui['border'] ),
			esc_attr( $ui['field_bg'] ?? '#FFFFFF' ),
			esc_attr( $ui['field_text'] ?? $ui['text'] ),
			esc_attr( $ui['button_text'] ?? '#FFFFFF' ),
			esc_attr( $ui['badge_bg'] ?? '#D92D20' ),
			esc_attr( $ui['badge_text'] ?? '#FFFFFF' ),
			esc_attr( $radius ),
			esc_attr( $shadow ),
			esc_attr( $list_width ),
			esc_attr( $max_width )
		...
		$categories  = get_terms( array( 'taxonomy' => self::CATEGORY, 'hide_empty' => false ) );
		$advanced    = $this->advanced_settings();
		$faq         = array_values( array_filter( (array) ( $advanced['faq'] ?? array() ), 'is_array' ) );

		$created_ticket_id = isset( $_GET['jarchi_ticket_created'] ) ? absint( $_GET['jarchi_ticket_created'] ) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$show_created_notice = false;
		if ( $created_ticket_id ) {
			$created_ticket = get_post( $created_ticket_id );
			$show_created_notice = $created_ticket instanceof \WP_Post
				&& self::POST_TYPE === $created_ticket->post_type
				&& (int) $created_ticket->post_author === get_current_user_id();
		}
		...
		if ( ! in_array( $filter, array( 'all', 'waiting', 'reviewing', 'answered', 'closed' ), true ) ) {
			$filter = 'all';
		}
		...
		$status_counts = array( 'all' => count( $all_items ), 'waiting' => 0, 'reviewing' => 0, 'answered' => 0, 'closed' => 0 );
		foreach ( $all_items as $filter_item ) {
			$state = $this->canonical_status( $this->status( (int) $filter_item->ID ) );
			if ( isset( $status_counts[ $state ] ) ) { ++$status_counts[ $state ]; }
		}
		$items = $all_items; // 1.23.0: render once; status/search filtering is instant in-page when JS is available.

		static $flow_instance = 0;
		++$flow_instance;
		$flow_id       = 'jarchi-ticket-flow-' . $flow_instance . '-' . get_current_user_id();
		$list_control  = $flow_id . '-list';
		$faq_control   = $flow_id . '-faq';
		$form_control  = $flow_id . '-form';
		$requested_view = isset( $_GET['jarchi_view'] ) ? sanitize_key( wp_unslash( (string) $_GET['jarchi_view'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$force_form     = isset( $_GET['jarchi_form'] ) && '1' === sanitize_text_field( wp_unslash( (string) $_GET['jarchi_form'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$ticket_error   = isset( $_GET['jarchi_ticket_error'] ) ? sanitize_key( wp_unslash( (string) $_GET['jarchi_ticket_error'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		if ( '' !== $ticket_error ) { $force_form = true; }
		$initial_state  = 'list';
		if ( $force_form ) {
			$initial_state = 'form';
		} elseif ( 'new' === $requested_view || 'new' === ( $overrides['view'] ?? '' ) ) {
			$initial_state = $faq ? 'faq' : 'form';
		}
		$new_target = $faq ? $faq_control : $form_control;

		$out  = '<div id="jarchi-tickets" class="jarchi-ticket-center jarchi-ticket-center--single' . esc_attr( $embedded_class ) . ( $ticket ? ' has-ticket-open' : '' ) . '" dir="rtl" data-initial-ticket-filter="' . esc_attr( $filter ) . '"' . ( $ticket ? ' data-jarchi-open-ticket="' . esc_attr( (string) $ticket->ID ) . '"' : '' ) . ' style="' . esc_attr( $ui_style ) . '">';
		$out .= '<input class="jarchi-ticket-state-control" type="radio" name="' . esc_attr( $flow_id ) . '" id="' . esc_attr( $list_control ) . '" data-jarchi-state-control="list"' . ( 'list' === $initial_state ? ' checked' : '' ) . ' />';
		$out .= '<input class="jarchi-ticket-state-control" type="radio" name="' . esc_attr( $flow_id ) . '" id="' . esc_attr( $faq_control ) . '" data-jarchi-state-control="faq"' . ( 'faq' === $initial_state ? ' checked' : '' ) . ' />';
		$out .= '<input class="jarchi-ticket-state-control" type="radio" name="' . esc_attr( $flow_id ) . '" id="' . esc_attr( $form_control ) . '" data-jarchi-state-control="form"' . ( 'form' === $initial_state ? ' checked' : '' ) . ' />';

		// Screen 1: ticket inbox.
		$out .= '<section class="jarchi-ticket-flow-panel jarchi-ticket-flow-panel--list" data-jarchi-flow-panel="list">';
		$wpep_unread = $this->unread_count();
		$out .= '<div class="jarchi-ticket-simple-head"><h2>' . esc_html( isset( $overrides['title'] ) ? (string) $overrides['title'] : __( 'تیکت‌های من', 'wp-event-publisher' ) ) . '</h2><div class="jarchi-ticket-head-actions">'
			. '<button type="button" class="jarchi-ticket-notify-enable" data-jarchi-enable-notifications hidden>' . $this->icon( 'bell' ) . '<span>' . esc_html__( 'فعال‌سازی اعلان', 'wp-event-publisher' ) . '</span></button>'
			. '<span class="jarchi-ticket-unread' . ( $wpep_unread > 0 ? ' is-visible' : '' ) . '" data-jarchi-ticket-unread aria-label="' . esc_attr__( 'پیام‌های خوانده‌نشده', 'wp-event-publisher' ) . '">' . esc_html( $wpep_unread > 99 ? '99+' : (string) $wpep_unread ) . '</span></div></div>';
		if ( $show_created_notice ) {
			$out .= '<div class="jarchi-ticket-created-notice" role="status" aria-live="polite" data-jarchi-ticket-created-notice>'
				. '<span class="jarchi-ticket-created-notice__icon">' . $this->icon( 'check' ) . '</span>'
				. '<span><strong>' . esc_html__( 'تیکت شما ثبت شد', 'wp-event-publisher' ) . '</strong><small>' . esc_html__( 'تیکت جدید در فهرست تیکت‌های شما قرار گرفت.', 'wp-event-publisher' ) . '</small></span>'
				. '</div>';
		}
		...
		foreach ( $filter_labels as $key => $data ) {
			$url = $base_url;
			if ( 'all' !== $key ) { $url = add_query_arg( 'ticket_status', $key, $url ); }
			$out .= '<a class="jarchi-ticket-filter ' . esc_attr( $data['class'] ) . ( $filter === $key ? ' is-active' : '' ) . '" href="' . esc_url( $url ) . '" data-jarchi-ticket-filter="' . esc_attr( $key ) . '" aria-current="' . ( $filter === $key ? 'page' : 'false' ) . '"><span class="jarchi-ticket-filter__icon">' . $this->icon( $data['icon'] ) . '</span><span class="jarchi-ticket-filter__label">' . esc_html( $data['label'] ) . '</span><span class="jarchi-ticket-filter__count">' . esc_html( (string) ( $status_counts[ $key ] ?? 0 ) ) . '</span></a>';
		}
		$out .= '</nav>';

		$new_button = isset( $overrides['new_button_text'] ) ? (string) $overrides['new_button_text'] : __( 'ثبت تیکت جدید', 'wp-event-publisher' );
		$out .= '<div class="jarchi-ticket-toolbar"><label class="jarchi-ticket-new" for="' . esc_attr( $new_target ) . '" role="button" tabindex="0" data-jarchi-flow-label>' . $this->icon( 'plus' ) . esc_html( $new_button ) . '</label>'
			. '<label class="jarchi-ticket-search"><span class="screen-reader-text">' . esc_html__( 'جستجوی تیکت‌ها', 'wp-event-publisher' ) . '</span>' . $this->icon( 'search' ) . '<input type="search" data-jarchi-ticket-list-search placeholder="' . esc_attr__( 'شماره یا موضوع تیکت را جستجو کنید…', 'wp-event-publisher' ) . '" autocomplete="off" /></label></div>';

		$out .= '<div class="jarchi-ticket-layout' . ( $ticket ? ' has-open-thread' : ' is-list-only' ) . '"><aside class="jarchi-ticket-list">';
		if ( ! empty( $items ) ) {
			$out .= '<div class="jarchi-ticket-list-head" aria-hidden="true"><span>' . esc_html__( 'موضوع', 'wp-event-publisher' ) . '</span><span class="jarchi-ticket-list-head__details"><span>' . esc_html__( 'شماره', 'wp-event-publisher' ) . '</span><span>' . esc_html__( 'دپارتمان', 'wp-event-publisher' ) . '</span><span>' . esc_html__( 'آخرین بروزرسانی', 'wp-event-publisher' ) . '</span><span>' . esc_html__( 'وضعیت', 'wp-event-publisher' ) . '</span></span></div>';
		}
		if ( empty( $items ) ) {
			$out .= '<div class="jarchi-ticket-empty"><strong>' . esc_html__( 'تیکتی در این بخش نیست', 'wp-event-publisher' ) . '</strong></div>';
		} else {
			foreach ( $items as $item ) {
				$unread = (bool) get_post_meta( $item->ID, '_jarchi_ticket_user_unread', true );
				$state  = $this->canonical_status( $this->status( (int) $item->ID ) );
				$terms  = wp_get_post_terms( (int) $item->ID, self::TAXONOMY );
				$department = ! is_wp_error( $terms ) && ! empty( $terms ) ? (string) $terms[0]->name : __( 'عمومی', 'wp-event-publisher' );
				$search_text = strtolower( wp_strip_all_tags( $item->post_title . ' ' . $item->ID . ' ' . $department . ' ' . $this->status_label( $state ) ) );
				$server_hidden = ( 'all' !== $filter && $state !== $filter ) ? ' hidden' : '';
				$out .= '<a class="jarchi-ticket-list-item' . ( $selected === (int) $item->ID ? ' is-active' : '' ) . ( $unread ? ' has-unread' : '' ) . '"' . $server_hidden . ' href="' . esc_url( add_query_arg( 'jarchi_ticket', (int) $item->ID, $base_url ) ) . '" data-ticket-id="' . esc_attr( (string) $item->ID ) . '" data-jarchi-ticket-row data-ticket-status="' . esc_attr( $state ) . '" data-search="' . esc_attr( $search_text ) . '">';
				$out .= '<span class="jarchi-ticket-list-main"><span class="jarchi-ticket-list-title"><strong>' . esc_html( $item->post_title ) . '</strong>' . ( $unread ? '<span class="jarchi-ticket-dot" aria-label="' . esc_attr__( 'پیام جدید', 'wp-event-publisher' ) . '"></span>' : '' ) . '</span><span class="jarchi-ticket-list-details">'
					. '<span><small>' . esc_html__( 'شماره تیکت', 'wp-event-publisher' ) . '</small><b>#' . esc_html( (string) $item->ID ) . '</b></span>'
					. '<span><small>' . esc_html__( 'دپارتمان', 'wp-event-publisher' ) . '</small><b>' . esc_html( $department ) . '</b></span>'
					. '<span><small>' . esc_html__( 'آخرین بروزرسانی', 'wp-event-publisher' ) . '</small><time>' . esc_html( get_the_modified_date( '', $item ) ) . '</time></span>'
					. '<span><small>' . esc_html__( 'وضعیت', 'wp-event-publisher' ) . '</small><span class="jarchi-ticket-chip jarchi-ticket-chip--' . esc_attr( $state ) . '">' . esc_html( $this->status_label( $state ) ) . '</span></span></span></span></a>';
			}
		}
		if ( ! empty( $items ) ) {
			$out .= '<div class="jarchi-ticket-empty jarchi-ticket-filter-empty" data-jarchi-ticket-filter-empty hidden><strong>' . esc_html__( 'تیکتی با این فیلتر پیدا نشد', 'wp-event-publisher' ) . '</strong></div>';
		}
		$out .= '</aside>';
		if ( $ticket ) { $out .= '<section class="jarchi-ticket-thread">' . $this->front_thread_markup( $ticket, $overrides, $base_url ) . '</section>'; }
		$out .= '</div></section>';

		// Screen 2: FAQ. This is intentionally a sibling of the inbox and form,
		// not a separate WordPress page. Radio/label state makes the flow work
		// even if a cache plugin drops every front-end JavaScript file.
		$out .= '<section class="jarchi-ticket-flow-panel jarchi-ticket-flow-panel--faq" data-jarchi-flow-panel="faq">';
		$out .= '<div class="jarchi-ticket-simple-head"><label class="jarchi-ticket-back-link jarchi-ticket-back-link--icon is-always-visible" for="' . esc_attr( $list_control ) . '" role="button" tabindex="0" data-jarchi-flow-label aria-label="' . esc_attr__( 'بازگشت به تیکت‌ها', 'wp-event-publisher' ) . '">' . $this->icon( 'back' ) . '</label><h2>' . esc_html__( 'ثبت تیکت جدید', 'wp-event-publisher' ) . '</h2></div>';
		$out .= $this->front_faq_markup( $faq, $form_control, $overrides );
		$out .= '</section>';

		// Screen 3: form only. The FAQ is completely replaced, not left above it.
		$out .= '<section class="jarchi-ticket-flow-panel jarchi-ticket-flow-panel--form" data-jarchi-flow-panel="form">';
		$out .= '<div class="jarchi-ticket-simple-head"><label class="jarchi-ticket-back-link jarchi-ticket-back-link--icon is-always-visible" for="' . esc_attr( $faq ? $faq_control : $list_control ) . '" role="button" tabindex="0" data-jarchi-flow-label aria-label="' . esc_attr__( 'بازگشت', 'wp-event-publisher' ) . '">' . $this->icon( 'back' ) . '</label><h2>' . esc_html__( 'ثبت تیکت جدید', 'wp-event-publisher' ) . '</h2></div>';
		$out .= '<section class="jarchi-ticket-thread jarchi-ticket-new-page">' . $this->front_ticket_form_markup( (array) $departments, (array) $categories, $overrides, $ticket_error, $base_url ) . '</section>';
		$out .= '</section></div>';
	...
	private function front_faq_markup( array $faq, string $form_control, array $overrides = array() ): string {
		$faq_title = isset( $overrides['faq_title'] ) ? (string) $overrides['faq_title'] : __( 'قبل از ثبت تیکت', 'wp-event-publisher' );
		$faq_description = isset( $overrides['faq_description'] ) ? (string) $overrides['faq_description'] : __( 'ممکن است پاسخ شما همین‌جا باشد.', 'wp-event-publisher' );
		$continue_text = isset( $overrides['faq_continue_text'] ) ? (string) $overrides['faq_continue_text'] : __( 'مشکلم حل نشد؛ ثبت تیکت', 'wp-event-publisher' );
		$out = '<div class="jarchi-ticket-form-card"><section class="jarchi-ticket-faq jarchi-ticket-faq--gate" data-jarchi-faq><div class="jarchi-ticket-faq__head">' . $this->icon( 'help' ) . '<div><strong>' . esc_html( $faq_title ) . '</strong><span>' . esc_html( $faq_description ) . '</span></div></div>';
		if ( $faq ) {
			$out .= '<div class="jarchi-ticket-faq__items">';
			foreach ( $faq as $item ) {
				$title = (string) ( $item['title'] ?? '' );
				$body  = (string) ( $item['body'] ?? '' );
				if ( '' === trim( $title ) ) { continue; }
				$out .= '<div class="jarchi-ticket-faq__item"><details><summary>' . esc_html( $title ) . '</summary><div>' . wp_kses_post( $body ) . '</div></details></div>';
			}
			$out .= '</div>';
		} else {
			$out .= '<div class="jarchi-ticket-empty jarchi-ticket-faq__empty"><strong>' . esc_html__( 'سؤال متداولی ثبت نشده است', 'wp-event-publisher' ) . '</strong><p>' . esc_html__( 'می‌توانید مستقیماً تیکت خود را ثبت کنید.', 'wp-event-publisher' ) . '</p></div>';
		}
		$out .= '<label class="jarchi-ticket-faq__continue" for="' . esc_attr( $form_control ) . '" role="button" tabindex="0" data-jarchi-flow-label>' . esc_html( $continue_text ) . '</label></section></div>';
		return $out;
	}

	private function front_ticket_form_markup( array $departments, array $categories, array $overrides = array(), string $ticket_error = '', string $base_url = '' ): string {
		$advanced = $this->advanced_settings();
		...
		$category_label = isset( $overrides['category_label'] ) && '' !== trim( (string) $overrides['category_label'] ) ? (string) $overrides['category_label'] : __( 'دسته‌بندی', 'wp-event-publisher' );
		$show_category = array_key_exists( 'show_category', $overrides ) ? (bool) $overrides['show_category'] : ! empty( $this->ui_settings()['show_category'] );
		$show_category = $show_category && ! empty( $categories );
		$out = '<div class="jarchi-ticket-form-card"><div id="jarchi-ticket-form" class="jarchi-ticket-form-shell is-visible" data-jarchi-ticket-form>';
		if ( '' !== $ticket_error ) {
			$error_messages = array(
				'expired'        => __( 'نشست این صفحه منقضی شده بود. فرم تازه شد؛ لطفاً دوباره ارسال کنید.', 'wp-event-publisher' ),
				'required'       => __( 'موضوع و متن تیکت را کامل کنید.', 'wp-event-publisher' ),
				'required_field' => __( 'لطفاً همه فیلدهای الزامی را کامل کنید.', 'wp-event-publisher' ),
				'not_saved'      => __( 'ثبت تیکت کامل نشد. دوباره تلاش کنید.', 'wp-event-publisher' ),
			);
			$error_message = $error_messages[ $ticket_error ] ?? __( 'ثبت تیکت انجام نشد. دوباره تلاش کنید.', 'wp-event-publisher' );
			$out .= '<div class="jarchi-ticket-inline-notice is-error" role="alert">' . $this->icon( 'warning' ) . '<strong>' . esc_html( $error_message ) . '</strong></div>';
		}
		$form_title = isset( $overrides['form_icon_title'] ) ? (string) $overrides['form_icon_title'] : __( 'تیکت جدید', 'wp-event-publisher' );
		$form_description = isset( $overrides['form_description'] ) ? (string) $overrides['form_description'] : __( 'موضوع را کوتاه و واضح بنویسید و جزئیات لازم را در پیام قرار دهید.', 'wp-event-publisher' );
		$out .= '<div class="jarchi-ticket-form-intro"><span class="jarchi-ticket-form-icon">' . $this->icon( 'support' ) . '</span><div><h3>' . esc_html( $form_title ) . '</h3><p>' . esc_html( $form_description ) . '</p></div></div>';
		$base_url = '' !== $base_url ? $this->clean_embed_url( $base_url ) : $this->ticket_page_url();
		$out .= '<form method="post" action="' . esc_url( $base_url ) . '" enctype="multipart/form-data" data-jarchi-ticket-submit-form>';
		$out .= '<input type="hidden" name="jarchi_front_action" value="submit" /><input type="hidden" name="jarchi_return_url" value="' . esc_attr( $base_url ) . '" /><input type="hidden" name="_wpnonce" value="' . esc_attr( wp_create_nonce( self::NONCE ) ) . '" />';
		$out .= '<label>' . esc_html( $subject_label ) . '<input type="text" name="title" required maxlength="180" autocomplete="off" /></label>';
		...
		foreach ( $departments as $department ) { if ( $department instanceof \WP_Term ) { $out .= '<option value="' . esc_attr( (string) $department->term_id ) . '">' . esc_html( $department->name ) . '</option>'; } }
		$out .= '</select></label>';
		if ( $show_category ) {
			$out .= '<label>' . esc_html( $category_label ) . '<select name="category"><option value="0">' . esc_html__( 'انتخاب کنید', 'wp-event-publisher' ) . '</option>';
			foreach ( $categories as $category ) { if ( $category instanceof \WP_Term ) { $out .= '<option value="' . esc_attr( (string) $category->term_id ) . '">' . esc_html( $category->name ) . '</option>'; } }
			$out .= '</select></label>';
		}
		$out .= '<label>' . esc_html( $priority_label ) . '<select name="priority"><option value="low">' . esc_html__( 'کم', 'wp-event-publisher' ) . '</option><option value="normal" selected>' . esc_html__( 'عادی', 'wp-event-publisher' ) . '</option><option value="high">' . esc_html__( 'بالا', 'wp-event-publisher' ) . '</option><option value="urgent">' . esc_html__( 'فوری', 'wp-event-publisher' ) . '</option></select></label></div>';
		foreach ( (array) ( $advanced['custom_fields'] ?? array() ) as $field ) {
			$key = sanitize_key( $field['key'] ?? '' ); $label = sanitize_text_field( $field['label'] ?? '' ); $type = $field['type'] ?? 'text';
			if ( ! $key || ! $label ) { continue; }
			$required = ! empty( $field['required'] ) ? ' required' : '';
			if ( 'textarea' === $type ) { $out .= '<label>' . esc_html( $label ) . '<textarea name="custom_field_' . esc_attr( $key ) . '" rows="3"' . $required . '></textarea></label>'; }
			elseif ( 'select' === $type ) { $out .= '<label>' . esc_html( $label ) . '<select name="custom_field_' . esc_attr( $key ) . '"' . $required . '><option value="">' . esc_html__( 'انتخاب کنید', 'wp-event-publisher' ) . '</option>'; foreach ( (array) ( $field['options'] ?? array() ) as $opt ) { $out .= '<option value="' . esc_attr( $opt ) . '">' . esc_html( $opt ) . '</option>'; } $out .= '</select></label>'; }
			else { $out .= '<label>' . esc_html( $label ) . '<input type="text" name="custom_field_' . esc_attr( $key ) . '"' . $required . ' /></label>'; }
		}
		$out .= '<label>' . esc_html( $message_label ) . '<textarea name="message" rows="7" required></textarea></label>';
		$policy = $this->attachment_policy();
		$note = sprintf( __( 'حداکثر %1$d فایل، هر فایل تا %2$d مگابایت.', 'wp-event-publisher' ), (int) $policy['max_files'], max( 1, (int) floor( $policy['max_bytes'] / MB_IN_BYTES ) ) );
		$out .= '<div class="jarchi-ticket-upload" data-jarchi-upload><input type="hidden" name="MAX_FILE_SIZE" value="' . esc_attr( (string) $policy['max_bytes'] ) . '" /><label class="jarchi-ticket-file jarchi-ticket-file--create">' . $this->icon( 'clip' ) . '<input type="file" name="attachments[]" accept="' . esc_attr( implode( ',', (array) $policy['mime'] ) ) . '" multiple data-jarchi-attachment-input data-max-files="' . esc_attr( (string) $policy['max_files'] ) . '" data-max-bytes="' . esc_attr( (string) $policy['max_bytes'] ) . '" data-max-total="' . esc_attr( (string) $policy['max_total_bytes'] ) . '" /><span>' . esc_html__( 'افزودن تصویر یا فایل', 'wp-event-publisher' ) . '</span></label><div class="jarchi-ticket-upload__preview" data-jarchi-upload-preview hidden></div><small>' . esc_html( $note ) . '</small><div class="jarchi-ticket-upload__error" data-jarchi-upload-error role="alert" hidden></div></div>';
		$out .= '<button type="submit" class="jarchi-ticket-submit">' . $this->icon( 'send' ) . esc_html__( 'ثبت تیکت', 'wp-event-publisher' ) . '</button></form></div></div>';
		return $out;
	}

	private function front_thread_markup( \WP_Post $ticket, array $overrides = array(), string $base_url = '' ): string {
		 ...
		 */
		$out = '<a class="jarchi-ticket-back-link jarchi-ticket-back-link--icon" href="' . esc_url( $base_url ) . '" aria-label="' . esc_attr__( 'بازگشت به تیکت‌ها', 'wp-event-publisher' ) . '" title="' . esc_attr__( 'بازگشت به تیکت‌ها', 'wp-event-publisher' ) . '">'
			. $this->icon( 'back' )
			. '</a>';

		if ( isset( $_GET['jarchi_reply_sent'] ) && '1' === sanitize_text_field( wp_unslash( (string) $_GET['jarchi_reply_sent'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$out .= '<div class="jarchi-ticket-inline-notice" role="status" data-jarchi-reply-sent><span>' . $this->icon( 'check' ) . '</span><strong>' . esc_html__( 'پاسخ شما ارسال شد', 'wp-event-publisher' ) . '</strong></div>';
		}
		if ( isset( $_GET['jarchi_reply_error'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$reply_error = sanitize_key( wp_unslash( (string) $_GET['jarchi_reply_error'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$reply_error_message = 'expired' === $reply_error
				? __( 'نشست این صفحه منقضی شده بود. صفحه تازه شد؛ لطفاً پاسخ را دوباره ارسال کنید.', 'wp-event-publisher' )
				: __( 'پاسخ ارسال نشد. دوباره تلاش کنید.', 'wp-event-publisher' );
			$out .= '<div class="jarchi-ticket-inline-notice is-error" role="alert" data-jarchi-reply-error><span>' . $this->icon( 'warning' ) . '</span><strong>' . esc_html( $reply_error_message ) . '</strong></div>';
		}
		...
		if ( $wpep_open && $wpep_answerable ) {
			$reply_policy = $this->attachment_policy();
			$out .= '<form class="jarchi-ticket-reply-form" method="post" action="' . esc_url( $base_url ) . '" enctype="multipart/form-data"><input type="hidden" name="jarchi_front_action" value="reply" /><input type="hidden" name="ticket_id" value="' . esc_attr( (string) $ticket->ID ) . '" /><input type="hidden" name="jarchi_return_url" value="' . esc_attr( $base_url ) . '" /><input type="hidden" name="_wpnonce" value="' . esc_attr( wp_create_nonce( self::NONCE ) ) . '" /><textarea name="message" rows="4" placeholder="' . esc_attr( isset( $overrides['reply_placeholder'] ) ? (string) $overrides['reply_placeholder'] : __( 'پاسخ خود را بنویسید…', 'wp-event-publisher' ) ) . '"></textarea><div class="jarchi-ticket-upload jarchi-ticket-upload--reply" data-jarchi-upload><div class="jarchi-ticket-reply-actions"><label class="jarchi-ticket-file">' . $this->icon( 'clip' ) . '<input type="file" name="attachments[]" accept="' . esc_attr( implode( ',', (array) $reply_policy['mime'] ) ) . '" multiple data-jarchi-attachment-input data-max-files="' . esc_attr( (string) $reply_policy['max_files'] ) . '" data-max-bytes="' . esc_attr( (string) $reply_policy['max_bytes'] ) . '" data-max-total="' . esc_attr( (string) $reply_policy['max_total_bytes'] ) . '" /><span>' . esc_html( isset( $overrides['attach_text'] ) ? (string) $overrides['attach_text'] : __( 'افزودن تصویر', 'wp-event-publisher' ) ) . '</span></label><button type="submit" class="jarchi-ticket-submit">' . esc_html( isset( $overrides['reply_text'] ) ? (string) $overrides['reply_text'] : __( 'ارسال پاسخ', 'wp-event-publisher' ) ) . '</button></div><div class="jarchi-ticket-upload__preview" data-jarchi-upload-preview hidden></div><div class="jarchi-ticket-upload__error" data-jarchi-upload-error role="alert" hidden></div></div></form>';
		} else {
			...
			$out .= '<div class="jarchi-ticket-locked">'
				...
				. '<a class="jarchi-ticket-submit" href="' . esc_url( add_query_arg( 'jarchi_view', 'new', $base_url ) ) . '">' . esc_html__( 'ثبت تیکت جدید', 'wp-event-publisher' ) . '</a>'
				. '</div>';
		}
		if ( 'closed' === $this->status( $ticket->ID ) && get_post_meta( $ticket->ID, '_jarchi_ticket_agent', true ) ) {
			...
			$out .= '<form class="jarchi-ticket-rating" method="post" action="' . esc_url( $base_url ) . '"><input type="hidden" name="jarchi_front_action" value="rating"><input type="hidden" name="ticket_id" value="' . esc_attr( (string) $ticket->ID ) . '"><input type="hidden" name="jarchi_return_url" value="' . esc_attr( $base_url ) . '"><input type="hidden" name="_wpnonce" value="' . esc_attr( wp_create_nonce( self::NONCE ) ) . '"><strong>امتیاز به پشتیبان</strong><div class="jarchi-ticket-stars">';
	...
	public function icon( string $name ): string {
		$paths = array(
			...
			'warning'   => '<path d="M12 3.5 21 19H3z"/><path d="M12 9v4.5M12 17h.01"/>',
		);
		...
		if ( is_readable( $js ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'newTicketUrl' => $this->new_ticket_page_url(),
					'listUrl'      => $this->ticket_page_url(),
				)
			);
		}
	}

	private function front_script(): string {
		return '<script>(function(){function init(root){if(!root||root.getAttribute("data-jarchi-flow-ready")==="1")return;root.setAttribute("data-jarchi-flow-ready","1");function sync(){var checked=root.querySelector("[data-jarchi-state-control]:checked");if(!checked)return;root.setAttribute("data-jarchi-flow-state",checked.getAttribute("data-jarchi-state-control")||"list");}root.addEventListener("change",function(e){if(e.target&&e.target.hasAttribute("data-jarchi-state-control")){sync();try{root.scrollIntoView({behavior:"smooth",block:"start"});}catch(err){root.scrollIntoView();}}});root.addEventListener("keydown",function(e){var label=e.target&&e.target.closest?e.target.closest("[data-jarchi-flow-label]"):null;if(!label||(e.key!=="Enter"&&e.key!==" "))return;e.preventDefault();var id=label.getAttribute("for");var control=id?document.getElementById(id):null;if(control){control.checked=true;control.dispatchEvent(new Event("change",{bubbles:true}));}});sync();}var roots=document.querySelectorAll(".jarchi-ticket-center--single");for(var i=0;i<roots.length;i++)init(roots[i]);})();</script>';
	}

	public function shortcode_icon( $atts = array() ): string {
		$atts = shortcode_atts(
			array( 'url' => '' ),
			(array) $atts,
			'jarchi_ticket_icon'
		...
		return $this->render_icon( (string) $atts['url'] );
	}

	public function render_icon( string $custom_url = '' ): string {
		$this->enqueue_front_assets();
		$count = is_user_logged_in() ? $this->unread_count() : 0;
		$url = '' !== trim( $custom_url ) ? esc_url_raw( $custom_url ) : $this->ticket_page_url();
		...
		return '<a class="jarchi-ticket-icon" href="' . esc_url( $url ) . '" data-jarchi-ticket-icon aria-label="' . esc_attr__( 'تیکت‌های من', 'wp-event-publisher' ) . '">' . $svg . '<span class="jarchi-ticket-icon__badge' . ( $count ? ' is-visible' : '' ) . '" data-jarchi-ticket-badge' . ( $count ? '' : ' hidden' ) . '>' . esc_html( (string) min( 99, $count ) ) . '</span></a>';
	}
