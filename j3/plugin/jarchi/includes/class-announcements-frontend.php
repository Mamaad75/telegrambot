...
class AnnouncementsFrontend {
	...
	/** User meta holding announcement IDs the current user has opened. */
	private const META_SEEN = '_jarchi_seen_announcements';

	/** Nonce action for marking announcement notifications as seen. */
	private const AJAX_NONCE = 'wpep_announcements_seen';
	...
	public function register(): void {
		...
		add_action( 'wp_ajax_wpep_announcements_seen', array( $this, 'ajax_mark_seen' ) );
	...
	public function icon_shortcode( $atts = array() ): string {
		...
		$active_ids = array_values( array_map( static fn( WP_Post $post ): int => (int) $post->ID, $items ) );
		$unread_ids = $this->unread_announcement_ids( $active_ids );
		$announcement_unread = count( $unread_ids );
		$ticket_unread = function_exists( 'wpep' ) ? wpep()->tickets()->unread_count() : 0;

		$bell_data = '';
		if ( is_user_logged_in() && ! empty( $active_ids ) ) {
			$bell_data = ' data-jarchi-announcement-ids="' . esc_attr( implode( ',', $active_ids ) ) . '"'
				. ' data-jarchi-announcement-ajax="' . esc_url( admin_url( 'admin-ajax.php' ) ) . '"'
				. ' data-jarchi-announcement-nonce="' . esc_attr( wp_create_nonce( self::AJAX_NONCE ) ) . '"';
		}

		$out  = $this->styles();
		$out .= '<div class="jarchi-bell" data-jarchi-bell' . $bell_data . '>';
		...
		$out .= '<span class="jarchi-bell__count' . ( $announcement_unread > 0 ? ' is-visible' : '' ) . '" data-jarchi-announcement-badge aria-hidden="true"' . ( $announcement_unread > 0 ? '' : ' hidden' ) . '>' . esc_html( (string) min( 99, $announcement_unread ) ) . '</span>';
		if ( $announcement_unread > 0 ) {
			$out .= '<span class="jarchi-bell__dot has-announcement-alert" data-jarchi-announcement-dot aria-label="' . esc_attr__( 'اطلاعیه جدید', 'wp-event-publisher' ) . '"></span>';
		}
	...
	/**
	 * Returns active announcement IDs the current user has not opened yet.
	 *
	 * Logged-out visitors have no account-scoped read state, so every active
	 * announcement is treated as unread for that anonymous session.
	 *
	 * @param int[] $active_ids Active announcement IDs.
	 * @return int[] Unread IDs.
	 */
	private function unread_announcement_ids( array $active_ids ): array {
		$active_ids = array_values( array_unique( array_filter( array_map( 'absint', $active_ids ) ) ) );
		if ( empty( $active_ids ) || ! is_user_logged_in() ) {
			return $active_ids;
		}

		$seen = get_user_meta( get_current_user_id(), self::META_SEEN, true );
		$seen = is_array( $seen ) ? array_values( array_unique( array_filter( array_map( 'absint', $seen ) ) ) ) : array();

		return array_values( array_diff( $active_ids, $seen ) );
	}

	/**
	 * Persists announcement read state when the current user opens the bell.
	 *
	 * @return void
	 */
	public function ajax_mark_seen(): void {
		if ( ! is_user_logged_in() ) {
			wp_send_json_error( array( 'message' => __( 'برای ثبت وضعیت خوانده‌شدن وارد حساب شوید.', 'wp-event-publisher' ) ), 401 );
		}

		check_ajax_referer( self::AJAX_NONCE, 'nonce' );

		$raw = isset( $_POST['ids'] ) ? wp_unslash( (string) $_POST['ids'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- checked above.
		$ids = array_values( array_unique( array_filter( array_map( 'absint', preg_split( '/\s*,\s*/', $raw ) ?: array() ) ) ) );

		// Only accept IDs that are announcements still active for this user.
		$active = $this->announcements->active( 50 );
		$allowed = array_map( static fn( WP_Post $post ): int => (int) $post->ID, $active );
		$ids = array_values( array_intersect( $ids, $allowed ) );

		$seen = get_user_meta( get_current_user_id(), self::META_SEEN, true );
		$seen = is_array( $seen ) ? array_values( array_unique( array_filter( array_map( 'absint', $seen ) ) ) ) : array();
		$seen = array_values( array_unique( array_merge( $seen, $ids ) ) );
		// Bound user meta forever; recent announcement IDs are enough for read state.
		if ( count( $seen ) > 500 ) {
			$seen = array_slice( $seen, -500 );
		}
		update_user_meta( get_current_user_id(), self::META_SEEN, $seen );
	...
	private function bell_script(): string {
		...
		return '<script>(function(){'
			. 'function clearAnnouncementBadge(b){var c=b.querySelector("[data-jarchi-announcement-badge]");if(c){c.textContent="0";c.hidden=true;c.classList.remove("is-visible");}var d=b.querySelector("[data-jarchi-announcement-dot]");if(d){d.remove();}}'
			. 'function markSeen(b){if(b.getAttribute("data-jarchi-announcement-sent")==="1"){return;}var ids=b.getAttribute("data-jarchi-announcement-ids")||"",url=b.getAttribute("data-jarchi-announcement-ajax")||"",nonce=b.getAttribute("data-jarchi-announcement-nonce")||"";if(!ids||!url||!nonce){return;}b.setAttribute("data-jarchi-announcement-sent","1");var body=new URLSearchParams();body.set("action","wpep_announcements_seen");body.set("nonce",nonce);body.set("ids",ids);fetch(url,{method:"POST",credentials:"same-origin",cache:"no-store",headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},body:body.toString()}).then(function(r){if(!r.ok){throw new Error("http");}return r.json();}).then(function(j){if(j&&j.success){clearAnnouncementBadge(b);}else{b.removeAttribute("data-jarchi-announcement-sent");}}).catch(function(){b.removeAttribute("data-jarchi-announcement-sent");});}'
			. 'document.addEventListener("click",function(e){var bells=document.querySelectorAll("[data-jarchi-bell]");bells.forEach(function(b){var btn=b.querySelector(".jarchi-bell__button"),p=b.querySelector(".jarchi-bell__panel");if(!btn||!p){return;}if(btn.contains(e.target)){var open=!p.hidden;p.hidden=open;btn.setAttribute("aria-expanded",String(!open));if(!open){markSeen(b);}}else if(!p.contains(e.target)){p.hidden=true;btn.setAttribute("aria-expanded","false");}});});'
			. '})();</script>';
	}
