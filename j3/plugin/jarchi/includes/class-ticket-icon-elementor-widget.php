...
class TicketIconElementorWidget extends \Elementor\Widget_Base {
    ...
    protected function register_controls(): void {
        ...
        $this->add_control( 'url', array( 'label' => __( 'لینک صفحه تیکت‌ها', 'wp-event-publisher' ), 'description' => __( 'صفحه‌ای را که خودتان با Elementor/JetEngine ساخته‌اید وارد کنید. اگر خالی باشد، آدرس اصلی ذخیره‌شده در تنظیمات ظاهر تیکت استفاده می‌شود.', 'wp-event-publisher' ), 'type' => \Elementor\Controls_Manager::URL, 'default' => array( 'url' => '' ), 'dynamic' => array( 'active' => true ) ) );
    ...
    protected function render(): void {
        ...
        $badge = ( 'yes' === ( $settings['show_badge'] ?? 'yes' ) )
            ? '<span class="jarchi-ticket-icon__badge' . ( $count ? ' is-visible' : '' ) . '" data-jarchi-ticket-badge' . ( $count ? '' : ' hidden' ) . ' style="' . esc_attr( $badge_style ) . '">' . esc_html( $count > 99 ? '99+' : (string) $count ) . '</span>'
            : '';
        ...
        echo '<a class="' . esc_attr( $widget_id ) . ' jarchi-ticket-icon jarchi-ticket-icon--elementor" href="' . esc_url( $url ) . '" aria-label="' . esc_attr__( 'تیکت‌های من', 'wp-event-publisher' ) . '" style="' . esc_attr( $style ) . '"><span class="jarchi-ticket-icon__elementor-inner">' . $icon_html . '</span>' . $badge . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
}
