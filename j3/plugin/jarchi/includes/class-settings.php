	...
	public function platform_defaults(): array {
		...
		foreach ( Field::PLATFORMS as $platform ) {
			$entry = array(
				...
				'contact_label'  => '',
				// 'author' keeps the original behaviour: the button reveals
				// the advertiser's own number and appears only when there is
				// one. 'support' points every advert at one fixed account, so
				// the button is always there and always reaches the same desk.
				'contact_mode'   => 'author',
				'contact_target' => '',
				// Separate from the button. A site can offer contact without
				// printing the number in the body of the message, which is the
				// usual arrangement once a support desk sits in between.
				'show_phone'     => true,
			);
