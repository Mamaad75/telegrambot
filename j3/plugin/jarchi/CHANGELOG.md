...
- حذف کامل پنل جداگانه «دسترسی طراح به متن قالب‌ها» از صفحه تیکت‌های خودکار.
- اضافه شدن دکمه «ویرایش» کنار اکشن‌های هر قانون؛ عنوان و متن همان تیکت در Popup ویرایش می‌شود و Trigger/شرط/Hook دست‌نخورده می‌ماند.
- نقش واقعی Designer/طراح بدون تنظیمات اضافه فقط دسترسی ویرایش متن می‌گیرد؛ مدیر سایت همچنان کنترل کامل دارد.
- تکمیل پروفایل اکنون فقط روی transition واقعی `incomplete -> complete` ارسال می‌شود و Metaهای ثبت‌نام دیگر تیکت «پروفایل تکمیل شد» را اشتباهی فعال نمی‌کنند.
- اتوماسیون «۷ روز مانده تا انقضای آگهی» و jobهای قدیمی آن حذف شدند؛ تیکت نهایی انقضای آگهی باقی است.
- تشخیص نردبان از Meta واقعی bump/boost/ladder/nardeban/featured/promotion به انتهای request موکول شد تا ذخیره چندفیلدی JetEngine باعث duplicate یا از دست رفتن تاریخ پایان نشود.
- شروع نردبان با fingerprint چرخه dedupe می‌شود؛ خرید نردبان جدید روی همان آگهی می‌تواند تیکت جدید بگیرد.
- هشدار نردبان بر اساس expiry واقعی دقیقاً ۴۸ ساعت قبل زمان‌بندی می‌شود و یک reconcile ساعتی زمان‌بندی‌های از دست‌رفته را بازیابی می‌کند.
- preset قدیمی «پایان نردبان» حذف و با «۲ روز تا پایان نردبان» جایگزین شده است.

## 1.26.5

- Added a restricted automatic-ticket template editor for designer/content roles.
- Administrators can choose which WordPress roles may edit adopted preset copy.
- Authorized designers can edit only ticket subject/body; triggers, conditions, enable state, departments, deletion, priorities and backend mechanics remain admin-only.
- Added a dedicated `jarchi_edit_ticket_templates` capability and a standalone designer menu.
- Existing preset-derived automations become editable after they are added; technical/custom rules stay protected.

## 1.26.4

- Fixed wp-admin Critical Error caused by the missing `upgrade_standard_pack_1262_comments()` init callback.
- Restored the comment/reply automation migration and kept the duplicate-email suppression.
- Preserved 1.26.3 ladder activation and the 48-hour pre-expiry reminder.
- Distribution folder renamed from `wp-event-publisher` to `jarchi`.
- Validated all `$this` action/filter callbacks and all PHP files.

# Changelog

## 1.26.3
- تیکت «آگهی شما نردبان شد» واقعاً داخل بسته استاندارد فعال شد؛ در 1.26.2 preset وجود داشت اما از لیست zero-config جا افتاده بود.
- Jarchi حالا تغییر meta مربوط به تاریخ پایان bump/boost/ladder را روی CPTهای عمومی تشخیص می‌دهد و در صورت وجود تاریخ آینده، رویداد نردبان را بدون تنظیم دستی راه‌اندازی می‌کند.
- با شروع هر چرخه نردبان، Jarchi تاریخ پایان آن را پیدا می‌کند و یک reminder مستقل دقیقاً ۲ روز قبل از پایان زمان‌بندی می‌کند.
- preset قدیمی «پایان نردبان» که در لحظه انقضا اجرا می‌شد برای بسته استاندارد غیرفعال و با «۲ روز تا پایان نردبان» جایگزین شد.
- چرخه‌های متعدد نردبان برای یک آگهی با `post_id + expiration timestamp` dedupe می‌شوند؛ بنابراین هر بار نردبان‌کردن جدید می‌تواند تیکت خودش را داشته باشد ولی یک چرخه دوبار ارسال نمی‌شود.
- متغیر `{bump_expiration_date}` برای متن تیکت/ایمیل اضافه شد.

## 1.26.1
- Added zero-config automated tickets for new approved comments on a user advert, advert update requests awaiting review, and user-initiated advert deletion.
- Profile completion is detected immediately from common front-end profile meta updates; the completion message includes the fields that are now filled.
- Added PublishPress Future integration for the 7-day expiry warning and final advert-expired ticket, with an hourly `_expiration-date` fallback for schedules that predate the upgrade.
- The standard Iran-Exim automation pack is enabled on upgrade; profile-incomplete moved from hourly scanning to registration-time detection to avoid bulk legacy-user notifications.
- Automated tickets now email the customer using the same subject/body and are also emitted to the Jarchi backend.
- Ticket webhook events carry top-level `post_id`, legacy `id`, and a structured `post` block to prevent `Missing post_id` regressions.
- Expired customer-form nonces redirect back into the same Ticket Center instead of displaying WordPress' generic expired-link screen.
- WP Rocket exclusions now also discover published pages containing the Ticket Center shortcode or Elementor widget, not only the configured ticket URL; `render_center()` enforces no-cache at runtime and 1.26.1 purges already-cached direct Ticket Center surfaces once.

# جارچی 1.26.0

- لیست تیکت کاربر داخل Widget واقعی Elementor از عرض ۷۰۰ پیکسل به بالا به جدول فشردهٔ یک‌خطی تبدیل شد؛ ستون‌های باریک و موبایل همچنان کارت امن و خوانا دارند.
- رنگ موضوع تیکت در لیست به‌صورت پیش‌فرض خاکستری تیره شد و در Elementor کنترل مستقل برای موضوع، متادیتا، سربرگ جدول، ردیف، Hover، فاصله ردیف و گردی جدول اضافه شد.
- کنترل‌های تایپوگرافی Elementor اکنون واقعاً عنوان «تیکت‌های من»، موضوع ردیف‌ها و متادیتای جدول را هدف می‌گیرند؛ selector قدیمی که به هدر ناموجود اشاره می‌کرد اصلاح شد.
- صفحه‌بندی مدیریت تیکت دیگر از `paginate_links()` و بازسازی مبهم query استفاده نمی‌کند؛ هر شماره صفحه مستقیماً با `Admin::app_url('support')` ساخته می‌شود و به داشبورد جارچی نمی‌پرد.
- تیکت‌های خودکار قدیمی یک‌بار repair می‌شوند: دپارتمان از قانون/قالب آماده بازیابی می‌شود، taxonomy تیکت اصلاح می‌شود و فرستنده به شکل «جارچی — نام دپارتمان» در متا و پیام نخست ثبت می‌شود.
- تشخیص تیکت خودکار در صندوق مدیریت علاوه بر origin، متای automation، شناسه automation و متای پیام نخست را هم بررسی می‌کند تا رکوردهای legacy به اشتباه «مدیریت سایت» نمایش داده نشوند.

# جارچی 1.25.0

- دو شاخهٔ 1.23.8 و 1.24.0 در یک نسخهٔ تجمعی مرج شدند؛ هیچ‌کدام از hardeningهای کش، nonce، پیوست، Badge و payload تیکت حذف نشده‌اند.
- فرستنده/گیرندهٔ تیکت‌های مدیریت بر اساس جهت واقعی مکالمه نمایش داده می‌شود؛ تیکت خودکار از نام دپارتمان خودش ارسال می‌شود.
- قالب‌های آمادهٔ اتوماسیون دپارتمان تخصصی را خودکار می‌سازند/پیدا می‌کنند و قانون‌های قدیمی بدون دپارتمان یک‌بار ترمیم می‌شوند.
- چهار اتوماسیون امن و رویدادمحور (ثبت‌نام، تأیید آگهی، رد آگهی، پاسخ دیدگاه) در صورت نبودن یک‌بار provision می‌شوند؛ قانون موجود یا انتخاب دستی مدیر overwrite نمی‌شود.
- FAQ دوباره فقط از صفحهٔ مستقل خودش مدیریت می‌شود و ذخیرهٔ تنظیمات پیشرفته دیگر FAQ را پاک نمی‌کند.
- لیست تیکت کاربر در دسکتاپ حالت فشردهٔ جدولی دارد و در موبایل/ستون باریک Elementor به کارت برمی‌گردد.
- صفحه‌بندی تیکت‌های مدیریت از حالت لیست عمودی خام خارج شد و pager افقی/ریسپانسیو دارد.
- گروه‌بندی منوی تیکت و کنترل دسته‌بندی فرم از 1.24.0 حفظ شده است.

# جارچی 1.24.0

- نسخه به 1.24.0 ارتقا یافت؛ این انتشار فقط bugfix نیست و ساختار مدیریت تیکت و قابلیت‌های طراح را بازآرایی می‌کند.
- منوی تیکت در سایدبار جارچی جمع‌وجور شد: «مدیریت پشتیبانی» شامل پشتیبان‌ها، دپارتمان‌ها، دسته‌بندی‌ها، سوالات متداول و پاسخ‌های آماده است.
- «تنظیمات تیکت» شامل ظاهر تیکت، پیامک، SLA و عملکرد، اعلان‌ها، ربات/Mini App و تنظیمات پیشرفته است.
- «اتوماسیون و نگهداری» شامل تیکت‌های خودکار و پاک‌سازی است؛ مسیرهای قدیمی و لینک‌های ذخیره‌شده همچنان سازگار می‌مانند.
- فیلد «دسته‌بندی» در فرم کاربر از این نسخه به‌صورت پیش‌فرض مخفی است و از «ظاهر تیکت‌ها» می‌توان آن را سراسری فعال کرد.
- Widget واقعی Elementor گزینه مستقل «دسته‌بندی در فرم: پیروی از تنظیمات / نمایش / مخفی» و برچسب قابل ویرایش دارد تا طراح برای هر صفحه تصمیم بگیرد.
- شورت‌کد `[jarchi_tickets]` نیز از `show_category="yes|no"` و `category_label="..."` پشتیبانی می‌کند.
- اگر هیچ دسته‌بندی‌ای تعریف نشده باشد، حتی در حالت فعال، select خالی به کاربر نمایش داده نمی‌شود.
- همه اصلاحات 1.23.8 شامل no-cache/nonce تازه، Preview پیوست، redirect داخل صفحه طراح، Badgeهای مستقل و payload صحیح تیکت حفظ شده‌اند.

# جارچی 1.23.8

- صفحه تیکت کاربر به‌صورت account-private از WP Rocket و page cache خارج شد؛ HTML قدیمی دیگر نباید فهرست تیکت یا nonce منقضی‌شده تحویل دهد و کش قدیمی صفحه نیز یک‌بار purge می‌شود.
- قبل از ثبت تیکت، پاسخ یا امتیاز، nonce فرم از endpoint همان‌مبدأ و احراز‌شده تازه می‌شود؛ در صورت خطا پیام داخل فرم نمایش داده می‌شود و کاربر به صفحه «پیوند منقضی شده» وردپرس پرت نمی‌شود.
- محدودیت فایل با سقف واقعی PHP/WordPress هماهنگ شد و مجموع حجم multipart نیز قبل از ارسال در مرورگر کنترل می‌شود.
- فایل‌های انتخاب‌شده قبل از ارسال Preview دارند: تصویر به‌صورت thumbnail و PDF/صوت/فایل با کارت کوچک و نام فایل.
- بعد از ایجاد/پاسخ، PRG همان صفحه Elementor/JetEngine را با cache-buster امن تازه می‌کند و پارامتر موقت از URL پاک می‌شود.
- Badge زنگ اطلاعیه دیگر با تیکت جمع نمی‌شود؛ وضعیت خوانده‌شدن اطلاعیه‌ها برای هر کاربر روی سرور ذخیره می‌شود و با بازکردن زنگ واقعاً صفر می‌شود. Badge تیکت فقط unread تیکت می‌ماند.
- Webhook رویدادهای `ticket.created` و `ticket.admin_replied` اکنون `contract_version` و `post_id` تیکت را در سطح اصلی payload می‌فرستد تا Backendهای سخت‌گیر هم آن را بپذیرند.
- قوانین قدیمی نردبان که از preset جارچی آمده‌اند و `hook_slug` یا role معتبر ندارند یک‌بار repair می‌شوند؛ preset «پایان نردبان» نیز اضافه شد.
- دکمه «فعال‌سازی بسته استاندارد جارچی» اضافه شد تا چهار Automation کم‌ریسک و رویدادمحور (خوش‌آمدگویی، تأیید آگهی، رد آگهی، پاسخ دیدگاه) بدون تنظیم دستی فعال شوند. اسکن‌های پروفایل عمداً خودکار روشن نمی‌شوند تا روی سایت موجود پیام انبوه نسازند.

# جارچی 1.23.7

- معماری Badge تیکت اصلاح شد: Badge آیکن، Badge منوی موبایل، شمارنده مرکز کاربری و عدد بالای مرکز تیکت همگی فقط از یک منبع واقعی یعنی `_jarchi_ticket_user_unread` تغذیه می‌شوند.
- باگ مهمی که اسکریپت اعلان‌ها تعداد Notificationهای عمومی را روی Badge تیکت می‌نوشت رفع شد؛ Notification و Ticket Badge دیگر هیچ‌وقت عدد یکدیگر را overwrite نمی‌کنند.
- باز کردن یک تیکت همان لحظه نقطه قرمز همان ردیف را حذف می‌کند و از طریق AJAX وضعیت read را روی سرور ثبت می‌کند؛ این رفتار در صفحات Cacheشده WP Rocket/Elementor/JetEngine هم درست می‌ماند.
- PHP همچنان هنگام Render تیکت را read می‌کند؛ مسیر JS لایه دفاعی اضافه برای cache و navigationهای صفحه‌ساز است.
- Notificationهای ذخیره‌شده `ticket_reply`/`ticket_created` با read شدن همان تیکت همزمان read می‌شوند و رکوردهای stale قدیمی نیز در summary بعدی خودکار reconcile می‌شوند.
- Badge ترکیبی زنگ اطلاعیه از Badge خالص تیکت جدا شد تا شمارنده تیکت دیگر با تعداد اطلاعیه‌های فعال جمع یا جایگزین نشود.
- شمارنده‌های وضعیت «در انتظار/بررسی/پاسخ داده شده/بسته شده» همچنان شمارنده وضعیت هستند و عمداً به unread تبدیل نشده‌اند.

# جارچی 1.23.6

- رفع باگ اصلی نگاشت پلتفرم‌ها: سوییچ‌های Telegram/Bale/WhatsApp اکنون منبع حقیقت هستند و مقدار legacy «مقصد ارسال» دیگر نمی‌تواند فیلدی را که برای تلگرام روشن شده بی‌صدا حذف کند.
- `DynamicPayload` فیلدهای روشن‌شده برای هر پلتفرم را حتی روی تنظیمات قدیمی/ناسازگار به Backend می‌فرستد تا `field_meta.platforms` واقعاً اعمال شود.
- قالب دستی و Preview نیز به‌جای `visibility=telegram` قدیمی از سوییچ واقعی Telegram استفاده می‌کنند.
- آیکن، پیشوند و پسوند فیلدها که در UI ذخیره می‌شدند ولی هنگام normalize از بین می‌رفتند اکنون واقعاً ذخیره و داخل `field_meta` ارسال می‌شوند.
- کنترل فنی و متناقض «مقصد ارسال» از UI حذف شد؛ Master switch و چیپ‌های پلتفرم تنها کنترل‌های انتشار هستند.
- این نسخه با Formatter v3 Backend جفت می‌شود؛ فیلدهای انتخاب‌شده با ترتیب/عنوان/آیکن خودشان در پیام داینامیک نمایش داده می‌شوند.

# جارچی 1.23.5

- باگ فرم «اختصاص پشتیبان» در صفحهٔ تکی تیکت رفع شد؛ فرم حالا صریحاً به `admin-post.php` ارسال می‌شود و handler واقعی `wpep_ticket_assign_agent` اجرا می‌شود.
- پس از تخصیص/حذف پشتیبان، مدیر روی همان تیکت می‌ماند و پیام موفقیت می‌بیند.
- انتخاب پشتیبان در سمت سرور اعتبارسنجی می‌شود و فقط کاربران دارای نقش «پشتیبان جارچی» قابل تخصیص هستند.
- اگر هنوز هیچ پشتیبانی تعریف نشده باشد، کنترل تخصیص به‌صورت گمراه‌کننده فعال نمی‌ماند و لینک مستقیم «مدیریت پشتیبان‌ها» نمایش داده می‌شود.

# جارچی 1.23.4

- رفع تیکت خودکار «تأیید/رد آگهی» برای CPTهای JetEngine و وضعیت‌های سفارشی؛ قالب آماده قدیمی که به اشتباه فقط روی `post` ذخیره شده بود یک‌بار اصلاح می‌شود.
- جستجوی مدیریت تیکت کامل شد: شناسه (با # و ارقام فارسی)، موضوع، متن گفتگو، نام، نام کاربری، ایمیل و موبایل.
- آدرس اصلی صفحه تیکت‌ها قابل تنظیم شد و آیکن Elementor/شورت‌کد دیگر هیچ‌وقت به مسیر حذف‌شده `/jarchi-tickets/` هدایت نمی‌شود.
- شورت‌کد آیکن از `url` پشتیبانی می‌کند و Widget Elementor دقیقاً همان لینک طراح را باز می‌کند.

# جارچی 1.23.3

- مرکز تیکت Elementor/JetEngine اکنون یک کامپوننت کاملاً Embedded است و لینک‌ها، بازکردن تیکت، پاسخ و ثبت تیکت جدید روی همان صفحه طراح باقی می‌مانند.
- فرم‌های کاربر آدرس برگشت امن همان صفحه میزبان را حمل می‌کنند؛ PRG بعد از ثبت/پاسخ دیگر کاربر را به صفحه اختصاصی تیکت جارچی نمی‌فرستد.
- جارچی دیگر هیچ صفحه WordPress برای تیکت به‌صورت خودکار ایجاد نمی‌کند؛ صفحات قدیمی حذف نمی‌شوند.
- واکنش‌گرایی المان تیکت بر پایه عرض Container اضافه شد تا در ستون‌های باریک JetEngine/Elementor روی دسکتاپ نیز متن، فیلتر، لیست و گفتگو متناسب شوند.

# جارچی 1.23.2

- پاسخ ادمین به تیکت دیگر به صفحه عمومی «تیکت‌های من» ریدایرکت نمی‌شود؛ همان تیکت داخل پنل وردپرس refresh می‌شود و وضعیت تازه بلافاصله دیده می‌شود.
- کنار عنوان «تیکت‌ها» در سایدبار داخلی ارتباط با کاربران، تعداد تیکت‌های باز (همه وضعیت‌ها به‌جز بسته‌شده) به‌صورت یک عدد مشکی ساده نمایش داده می‌شود.

# جارچی 1.23.1

- رفع باگ حیاتی ثبت تیکت کاربران عادی: فرم‌های کاربری دیگر به `wp-admin/admin-post.php` ارسال نمی‌شوند و مستقیماً روی صفحه عمومی مرکز تیکت پردازش می‌شوند.
- سازگاری با افزونه‌های عضویت/پروفایل که کاربران Subscriber را از wp-admin به «حساب کاربری» ریدایرکت می‌کنند.
- مسیر عمومی مشابه برای پاسخ کاربر و ثبت امتیاز پشتیبان، تا این عملیات هم تحت تأثیر محدودکننده‌های wp-admin قرار نگیرند.
- پل سازگاری زودهنگام برای فرم‌های کش‌شده نسخه‌های قبلی؛ درخواست قدیمی admin-post برای کاربر عادی قبل از ریدایرکت‌های رایج مدیریت می‌شود.
- رفتار موفقیت حفظ شد: بعد از ثبت واقعی تیکت، کاربر به «تیکت‌های من» برمی‌گردد و پیام موفقیت فقط پس از ذخیره کامل تیکت/پیام نمایش داده می‌شود.

# جارچی 1.23.0

- بازطراحی نهایی نمای گفتگو: هنگام باز شدن تیکت، سایدبار/فیلترهای inbox پنهان می‌شوند و گفتگو تمام عرض در اختیار دارد تا تداخل و روی‌هم‌افتادن متن‌ها از بین برود.
- پیام‌های کاربر به‌صورت حباب چت در سمت چپ و پاسخ‌های پشتیبانی/ادمین در سمت راست نمایش داده می‌شوند؛ پیوست‌ها، متادیتا و فرم پاسخ نیز responsive شده‌اند.
- فیلترهای وضعیت و جستجوی «تیکت‌های من» بدون reload و بدون پرش صفحه کار می‌کنند و URL را فقط با History API به‌روز می‌کنند.
- redirect ثبت تیکت و پاسخ canonical شد: ثبت موفق به inbox برمی‌گردد، پاسخ موفق همان گفتگو را باز می‌کند و خطاهای ثبت/پاسخ به state صحیح همان مرکز تیکت هدایت می‌شوند.
- خطای ذخیره پیام پاسخ دیگر به‌اشتباه پیام موفقیت نشان نمی‌دهد.
- صفحه تنظیمات مستقل «تنظیمات تیکت» از منوی جارچی حذف شد و گزینه‌های فعال‌سازی Telegram/Bale Mini App به «پیشرفته تیکت» منتقل شدند؛ مسیر قدیمی برای سازگاری به صفحه جدید redirect می‌شود.
- صفحه «پیشرفته تیکت» در دسکتاپ، تبلت و موبایل responsive شد و افزودن فیلد سفارشی اکنون ورودی گزینه‌های select را نیز درست ایجاد می‌کند.
- جریان تک‌صفحه‌ای inbox → FAQ → فرم ثبت تیکت حفظ شده و برای keyboard نیز Enter/Space روی کنترل‌های state پشتیبانی می‌شود.
- REST تیکت برای Backend/Mini App همچنان WordPress را منبع اصلی داده نگه می‌دارد؛ این نسخه با Backend 2.4.15 برای پاسخ‌گویی staff از Mini App جفت شده است.

# جارچی 1.22.1

- بعد از ثبت موفق تیکت، کاربر به همان نمای «تیکت‌های من» برمی‌گردد؛ دیگر ترد تیکت به‌صورت خودکار باز نمی‌شود.
- پیام موفقیت «تیکت شما ثبت شد» داخل همان مرکز تیکت نمایش داده می‌شود.
- پیام موفقیت پس از چند ثانیه محو می‌شود و پارامتر موفقیت از URL پاک می‌شود تا با Refresh دوباره نمایش داده نشود.
- نمایش پیام فقط پس از اعتبارسنجی مالکیت تیکت انجام می‌شود.

# جارچی 1.22.0

- ادغام «تیکت‌های من»، سوالات متداول و فرم ثبت تیکت در یک صفحه و یک ویجت المنتور.
- تغییر حالت‌ها بدون باز کردن برگه جدید: تیکت جدید → FAQ → فرم ثبت تیکت.
- سوئیچ حالت بر پایه radio/label و CSS است تا حتی در صورت اختلال WP Rocket/Elementor/JS نیز کلیک‌ها کار کنند.
- صفحه جداگانه `jarchi-new-ticket` دیگر ساخته یا برای مسیر جدید استفاده نمی‌شود؛ لینک‌های قدیمی به همان صفحه اصلی تیکت هدایت می‌شوند.
- متن عنوان FAQ، توضیح FAQ و دکمه ادامه از خود ویجت Elementor قابل ویرایش است.
- کلیدهای Back در FAQ و فرم به حالت قبلی همان صفحه برمی‌گردند.

# جارچی 1.21.2

- رفع قطعی دکمه «تیکت جدید» در سایت‌هایی که شناسه برگه لیست و برگه ثبت تیکت به اشتباه یکی شده بود.
- مسیر ثبت تیکت اکنون همیشه `jarchi_view=new` را حمل می‌کند تا تنظیم ثابت ویجت Elementor نتواند آن را خنثی کند.
- کلیک CTA در capture phase از تداخل AJAX navigation قالب/صفحه‌ساز محافظت می‌شود و همچنان fallback لینک واقعی دارد.
- دکمه «مشکلم حل نشد؛ ثبت تیکت» فقط وقتی فرم داخل DOM پیدا شود `preventDefault()` می‌کند؛ در غیر این صورت fallback سروری `jarchi_form=1` اجرا می‌شود.

# جارچی 1.21.1

- رفع کامل باگ دکمه «ثبت تیکت جدید» در Widget المنتور: پارامتر `jarchi_view=new` حالا بر تنظیم ثابت Widget اولویت دارد.
- دکمه «مشکلم حل نشد؛ ثبت تیکت» به progressive enhancement تبدیل شد؛ حتی اگر JavaScript توسط کش/بهینه‌ساز/Elementor اجرا نشود، لینک مستقیم فرم را باز می‌کند.
- JavaScript مرکز تیکت دیگر در نبود `wpepTickets` به‌طور کامل متوقف نمی‌شود؛ FAQ و نمایش فرم مستقل از AJAX badge کار می‌کنند.
- حذف interception غیرضروری دکمه تیکت جدید و اتکا به navigation استاندارد مرورگر برای سازگاری بیشتر.

# جارچی 1.21.0

نسخهٔ Elementor-native تیکت‌ها، فیلدهای runtime و شماره تماس نویسنده آگهی.

## Elementor و تیکت سمت کاربر
- صفحات «تیکت‌های من» و «ثبت تیکت جدید» دیگر برای ساخت جدید به شورت‌کد وابسته نیستند و Widget واقعی `Jarchi Ticket Center` در Elementor می‌گیرند.
- صفحات Elementor قدیمی که فقط Widget شورت‌کد جارچی دارند، همان Widget را درجا به Widget بومی جارچی تبدیل می‌کنند و چیدمان اطراف کاربر حفظ می‌شود.
- صفحهٔ لیست تیکت‌ها به صندوق تیکت متراکم‌تر با شماره، موضوع، دپارتمان، وضعیت، آخرین بروزرسانی و جستجوی محلی تبدیل شد.
- ثبت تیکت در صفحهٔ جدا انجام می‌شود؛ FAQ بدون نوار جستجو نمایش داده می‌شود و دکمهٔ «مشکلم حل نشد؛ ثبت تیکت» فرم واقعی را باز می‌کند.
- دکمهٔ بازگشت صفحهٔ ثبت/گفتگو فقط فلش است و همهٔ اجزا برای موبایل و Elementor responsive شده‌اند.

## فیلدهای داینامیک
- discovery سراسری با اسکن runtime چند نوشتهٔ واقعی در scope انتخاب‌شده تکمیل می‌شود.
- فیلدهای runtime کشف‌شده در همان request به registry افزوده می‌شوند تا در mapping قابل ذخیره باشند.
- هنگام انتشار، فیلدهای واقعی همان نوشته دوباره resolve می‌شوند؛ JetEngine/ACF/post metaهای جدید تا وقتی مدیر آن‌ها را برای انتشار فعال کند قابل استفاده‌اند.

## شماره تماس و Contract 1.4
- شمارهٔ دکمهٔ تماس از user meta نویسندهٔ آگهی خوانده می‌شود، نه از فیلد شمارهٔ داخل آگهی.
- کلیدهای رایج WooCommerce/JetEngine مانند `billing_phone`, `phone`, `mobile`, `phone_number` و ... پشتیبانی می‌شوند و با filter قابل توسعه‌اند.
- شماره فقط برای پلتفرمی وارد payload می‌شود که دکمهٔ تماس آن روشن باشد؛ فیلد عمومی `phone` آگهی همچنان policy مستقل خودش را دارد.
- قرارداد webhook به 1.4 ارتقا یافت و `contact_phone` + `contact_phone_platforms` را حمل می‌کند.

---

# جارچی 1.20.0

نسخهٔ بازطراحی تیکت و کشف فیلدهای داینامیک.

## فیلدهای انتشار
- بارگذاری اسکریپت صفحهٔ فیلدها روی مسیر جدید `jarchi_view=fields` اصلاح شد؛ صفحه دیگر روی «در حال بارگذاری» نمی‌ماند.
- JetEngine از APIهای مختلف و در صورت نیاز از تنظیمات ذخیره‌شدهٔ meta box فیلدها را کشف می‌کند.
- فیلدهای واقعی همان نوشته هنگام انتشار نیز اسکن می‌شوند؛ فیلدهای کم‌استفاده یا داینامیک دیگر فقط به خاطر نبودن در کش سراسری حذف نمی‌شوند.
- کلیدهای کاملاً غیرلاتین شناسهٔ پایدار می‌گیرند و از فهرست فیلدها حذف نمی‌شوند.
- سقف اسکن meta از 300 به 1000 کلید افزایش یافت و خطای AJAX با پیام قابل فهم برمی‌گردد.

## مرکز کاربری
- ساخت خودکار صفحهٔ «مرکز کاربری جارچی» حذف شد.
- در تنظیمات ظاهر تیکت، مدیر می‌تواند صفحه را فقط در صورت نیاز بسازد و «فقط اعلانات»، «فقط تیکت‌ها» یا «هر دو» را انتخاب کند.
- صفحهٔ قدیمی که افزونه خودکار ساخته و دست‌نخورده مانده باشد یک‌بار به زباله‌دان منتقل می‌شود؛ صفحات ویرایش‌شده یا Elementor دست نمی‌خورند.

## تیکت سمت کاربر
- صفحهٔ اصلی فقط صندوق تیکت‌هاست و یک دکمهٔ «ثبت تیکت جدید» دارد.
- ثبت تیکت به صفحهٔ جدا منتقل شد.
- FAQ قبل از فرم نمایش داده می‌شود و فرم پس از «مشکلم حل نشد؛ ثبت تیکت» باز می‌شود.
- گفتگوها خواناتر، موبایل‌پسندتر و با پاسخ چسبیده و حباب‌های واضح‌تر نمایش داده می‌شوند.
- فیلترهای موبایل بدون اسکرول‌اند: «همه» تمام‌عرض و چهار وضعیت دیگر 2×2.

## مدیریت تیکت
- فهرست مدیریت به جدول واقعی با ID، موضوع، فرستنده، گیرنده، دپارتمان، اولویت، وضعیت و تاریخ تبدیل شد.
- جدول روی موبایل به کارت‌های خوانا تبدیل می‌شود و وضعیت‌ها نیز بدون اسکرول نمایش داده می‌شوند.
- صفحه‌بندی به فهرست مدیریت اضافه شد.

## پاسخ آماده و Badge
- decode پاسخ‌های آماده برای UTF-8/Persian اصلاح شد و متن‌های mojibake قدیمی به‌صورت محافظه‌کارانه تعمیر می‌شوند.
- باگ «افزودن پاسخ آماده به FAQ» که ساختار کل تنظیمات را داخل FAQ می‌گذاشت اصلاح شد.
- Badge از `COUNT(DISTINCT ticket)` می‌خواند تا meta تکراری عدد را چند برابر نکند؛ کش آن کوتاه‌تر و Widget Elementor نیز در حالت صفر صحیح است.
- خطای متغیر تعریف‌نشده در Widget آیکن تیکت حذف شد.
