...
/*
 * WordPress calls this class WP_User, and the plugin typehints against it. An
 * alias is the same class rather than a subclass, so every object already in
 * the store satisfies a ?WP_User parameter — which is what makes the typehints
 * real here instead of unreachable.
 */
class_alias( 'WP_User_Stub', 'WP_User' );
...
class FakeWpdb {
	...
	public function update( $t, $d, $w, $df = null, $wf = null ) {
		if ( str_contains( (string) $t, 'wpep_automation_events' ) ) {
			$key = (string) ( $w['event_key'] ?? '' );

			if ( ! isset( $GLOBALS['ledger'][ $key ] ) ) { return 0; }

			$GLOBALS['ledger'][ $key ] = array_merge( $GLOBALS['ledger'][ $key ], (array) $d );
	...
	public function delete( $t, $w, $f = null ) {
		if ( str_contains( (string) $t, 'wpep_automation_events' ) ) {
			$key = (string) ( $w['event_key'] ?? '' );
			$row = $GLOBALS['ledger'][ $key ] ?? null;

			if ( ! $row ) { return 0; }
			if ( isset( $w['state'] ) && $row['state'] !== $w['state'] ) { return 0; }

			unset( $GLOBALS['ledger'][ $key ] );
	...
	/*
	 * Models INSERT IGNORE against a UNIQUE index, plus the conditional
	 * UPDATE and the DELETEs the ledger relies on.
	 *
	 * This is the most important stub in the file. The whole anti-duplication
	 * design rests on the database refusing a second row for the same key and
	 * saying so through rows-affected. A query() that returns a constant would
	 * make "two concurrent callers cannot both win" pass without any of that
	 * being true — which is precisely the shape of the bug being fixed.
	 */
	public function query( $sql ) {
		$this->queries[] = $sql;
		$sql             = (string) $sql;

		if ( ! str_contains( $sql, 'wpep_automation_events' ) ) {
			return 0;
		}

		if ( ! isset( $GLOBALS['ledger'] ) ) { $GLOBALS['ledger'] = array(); }

		if ( str_starts_with( ltrim( $sql ), 'INSERT IGNORE' ) ) {
			if ( ! preg_match_all( "/'([^']*)'/", $sql, $m ) ) { return 0; }

			$values = $m[1];
			$key    = $values[0] ?? '';

			// The UNIQUE index: a key that is present is not inserted again,
			// and the caller is told nothing happened.
			if ( '' === $key || isset( $GLOBALS['ledger'][ $key ] ) ) { return 0; }

			// user_id is the %d placeholder, so it is the unquoted value.
			preg_match( '/,\s*(\d+)\s*,/', $sql, $uid );

			$GLOBALS['ledger'][ $key ] = array(
				'key'        => $key,
				'rule_id'    => $values[1] ?? '',
				'user_id'    => (int) ( $uid[1] ?? 0 ),
				'event_type' => $values[2] ?? '',
				'object_id'  => $values[3] ?? '',
				'state'      => $values[4] ?? 'pending',
				'ticket_id'  => 0,
				'created_at' => $values[5] ?? gmdate( 'Y-m-d H:i:s' ),
			);
		...
		if ( str_starts_with( ltrim( $sql ), 'UPDATE' ) ) {
			// The stale-pending takeover. Only a row that is pending AND older
			// than the cutoff may be adopted, and only once.
			if ( ! preg_match_all( "/'([^']*)'/", $sql, $m ) ) { return 0; }
			...
			if ( ! $row || $row['state'] !== $state || $row['created_at'] >= $cutoff ) { return 0; }

			$GLOBALS['ledger'][ $key ]['created_at'] = $now;

			return 1;
		}

		if ( str_starts_with( ltrim( $sql ), 'DELETE' ) ) {
			$before = count( $GLOBALS['ledger'] );

			if ( preg_match( "/rule_id = '([^']*)'/", $sql, $m ) ) {
				foreach ( $GLOBALS['ledger'] as $k => $row ) {
					if ( $row['rule_id'] === $m[1] ) { unset( $GLOBALS['ledger'][ $k ] ); }
				}
			} elseif ( preg_match( "/created_at < '([^']*)'/", $sql, $m ) ) {
				foreach ( $GLOBALS['ledger'] as $k => $row ) {
					if ( $row['created_at'] < $m[1] ) { unset( $GLOBALS['ledger'][ $k ] ); }
				}
			} else {
				$GLOBALS['ledger'] = array();
			}

			return $before - count( $GLOBALS['ledger'] );
		}
	...
	public function get_var( $sql ) {
		...
		if ( str_contains( (string) $sql, 'wpep_automation_events' ) ) {
			...
			if ( str_contains( (string) $sql, 'COUNT(*)' ) ) { return count( $GLOBALS['ledger'] ); }

			if ( preg_match( "/event_key = '([^']*)'/", (string) $sql, $m ) ) {
				return isset( $GLOBALS['ledger'][ $m[1] ] ) ? 1 : 0;
			}
	...
	/*
	 * Substitutes placeholders the way WordPress does: by the placeholder's
	 * type, not the value's.
	 *
	 * Quoting on is_numeric() instead meant a numeric object id came back
	 * unquoted, which shifted every column after it and silently corrupted the
	 * row the ledger model read back.
	 */
	public function prepare( $sql, ...$a ) {
		...
		while ( '' !== $sql ) {
			if ( ! preg_match( '/%[sdfF]/', $sql, $m, PREG_OFFSET_CAPTURE ) ) {
				$out .= $sql;
				break;
			}
			...
			} elseif ( '%f' === $token || '%F' === $token ) {
				$out .= (string) (float) $value;
			} else {
				$out .= "'" . str_replace( "'", "\\'", (string) $value ) . "'";
			}

			$sql = substr( $sql, $at + strlen( $token ) );
		}
...
function rest_sanitize_boolean( $v ) {
	if ( is_string( $v ) ) { return ! in_array( strtolower( trim( $v ) ), array( '', '0', 'false', 'off', 'no' ), true ); }
	return (bool) $v;
}
...
function clean_post_cache( $post ) { $id = is_object( $post ) ? (int) $post->ID : (int) $post; wp_cache_delete( $id, 'posts' ); }
function clean_comment_cache( $ids ) { foreach ( (array) $ids as $id ) { wp_cache_delete( (int) $id, 'comment' ); } }
...
function get_post_time( $f = 'U', $gmt = false, $p = null, $translate = false ) { return 'U' === $f ? time() : gmdate( (string) $f ); }
function get_post_modified_time( $f = 'U', $gmt = false, $p = null, $translate = false ) { return get_post_time( $f, $gmt, $p, $translate ); }
...
function get_term_by( $field, $value, $tax = '', $output = OBJECT ) {
	foreach ( $GLOBALS['terms'] as $t ) {
		if ( '' !== $tax && $t->taxonomy !== $tax ) { continue; }
		$have = 'slug' === $field ? ( $t->slug ?? '' ) : ( 'name' === $field ? ( $t->name ?? '' ) : ( $t->term_id ?? 0 ) );
		if ( (string) $have === (string) $value ) { return $t; }
	}
	return false;
}
...
function get_users( $args = array() ) {
	...
	// Ordering and offset are what the cursor-based scheduled scan depends on;
	// ignoring them would make "the batch resumes where it stopped" pass while
	// the same users were examined every run.
	if ( 'ID' === ( $args['orderby'] ?? '' ) ) {
		usort( $users, static fn( $a, $b ) => (int) $a->ID <=> (int) $b->ID );
	...
	if ( ! empty( $args['offset'] ) ) {
		$users = array_slice( $users, (int) $args['offset'] );
	}

	if ( ! empty( $args['number'] ) && (int) $args['number'] > 0 ) {
...
/**
 * Reads the file's real signature, the way WordPress does.
 *
 * A stub that trusted the extension would let every content-sniffing check in
 * the plugin pass without any sniffing existing — a .php file renamed to
 * .png would be accepted by the test for the same reason it would be accepted
 * in production. So this opens the temporary file and reads the first bytes.
 *
 * @param string $file     Path on disk.
 * @param string $filename Claimed name.
 *
 * @return array{ext:string|false,type:string|false,proper_filename:string|false} Result.
 */
function wp_check_filetype_and_ext( $file, $filename, $mimes = null ) {
	...
	$type    = $ext_to_type[ $claimed ] ?? false;
	$ext     = $type ? $claimed : false;

	$bytes = is_readable( (string) $file ) ? (string) file_get_contents( (string) $file, false, null, 0, 16 ) : '';

	$signatures = array(
		"\x89PNG"        => array( 'png', 'image/png' ),
		"\xFF\xD8\xFF"   => array( 'jpg', 'image/jpeg' ),
		'GIF8'           => array( 'gif', 'image/gif' ),
		'%PDF'           => array( 'pdf', 'application/pdf' ),
		'RIFF'           => array( 'wav', 'audio/wav' ),
		'ID3'            => array( 'mp3', 'audio/mpeg' ),
		'OggS'           => array( 'ogg', 'audio/ogg' ),
		'PK'             => array( 'zip', 'application/zip' ),
	);

	$real = null;

	foreach ( $signatures as $magic => $pair ) {
		if ( str_starts_with( $bytes, $magic ) ) { $real = $pair; break; }
	}

	// Nothing recognisable — a text or script payload. WordPress reports no
	// usable type for these, which is what makes them refusable.
	if ( null === $real ) {
		return array( 'ext' => false, 'type' => false, 'proper_filename' => false );
	}

	// Contents and extension disagree: report the truth and a corrected name.
	if ( $real[1] !== $type ) {
		$base = (string) pathinfo( (string) $filename, PATHINFO_FILENAME );

		return array( 'ext' => $real[0], 'type' => $real[1], 'proper_filename' => $base . '.' . $real[0] );
	}
...
function wp_check_filetype( $filename, $mimes = null ) {
	$r = wp_check_filetype_and_ext( '', $filename );

	return array( 'ext' => $r['ext'], 'type' => $r['type'] );
}

/*
 * is_uploaded_file() is a PHP builtin that answers "did this request POST
 * this file?", and under the CLI the answer is always no — so left alone it
 * would refuse every fixture and the upload tests would pass for the wrong
 * reason. PHP resolves an unqualified call inside a namespace to that
 * namespace first, so a WPEventPublisher\is_uploaded_file() substitutes a
 * version that consults a list the test controls. It lives in its own file
 * because a namespace declaration cannot follow code. The plugin is
 * unchanged and still reaches the real builtin in production.
 */
require_once __DIR__ . '/harness-upload-shim.php';

function wp_delete_attachment( $id, $force = false ) {
	$id = (int) $id;

	if ( ! isset( $GLOBALS['attachments'][ $id ] ) ) { return false; }

	unset( $GLOBALS['attachments'][ $id ], $GLOBALS['posts'][ $id ] );
...
function has_blocks( $p = null ) { return false; }

function wp_trim_words( $text, $num = 55, $more = null ) { $w = preg_split( '/\s+/u', trim( (string) $text ) ); return implode( ' ', array_slice( (array) $w, 0, (int) $num ) ); }

function strip_shortcodes( $content ) { return preg_replace( '/\[[^\]]*\]/', '', (string) $content ); }

function set_url_scheme( $url, $scheme = null ) { return preg_replace( '#^https?://#i', 'https://', (string) $url ); }
