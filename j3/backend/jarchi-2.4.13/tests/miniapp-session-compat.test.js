...
test("2.4.13 preserves legacy session query compatibility and safe cookie exchange", () => {
...
test("Telegram keeps its loader while Bale SDK is first but non-blocking", () => {
  ...
  assert.match(html, /sdk-loader\.js\?v=2\.4\.13/);
  ...
  const appScriptPos = baleHtml.indexOf('/app/app-bale-legacy.js?v=2.4.13');
...
test("Bale dedicated entry is a real Mini App document with SDK first", () => {
  ...
  assert.match(baleHtml, /\/app\/app-bale-legacy\.js\?v=2\.4\.13/);
