...
SCREENS.fields = async function fields() {
  ...
  skeleton(4);
  const editable = hasFeature("site_control") && (["owner", "admin"].includes(siteRole()) || can("fields.manage"));
  const data = editable
  ...
  wireFieldControls(site.id, list, () => go("fields"));
};
...
function wireFieldControls(siteId, list, refresh) {
  $$("[data-field-platform]").forEach((input) => {
    input.onchange = async () => {
      input.disabled = true;
      try {
        await adminApi(`/clients/${encodeURIComponent(siteId)}/fields/${encodeURIComponent(input.dataset.fieldKey)}`, {
          method: "PATCH", body: JSON.stringify({ platforms: { [input.dataset.fieldPlatform]: input.checked } }),
        });
        toast("بروزرسانی شد");
      } catch (error) {
  ...
  $$("[data-field-hidden]").forEach((input) => {
    input.onchange = async () => {
      input.disabled = true;
      try {
        await adminApi(`/clients/${encodeURIComponent(siteId)}/fields/${encodeURIComponent(input.dataset.fieldHidden)}`, {
  ...
  $$("[data-field-label]").forEach((button) => {
    button.onclick = () => modal({
      ...
      onSubmit: async (values) => {
        ...
        await adminApi(`/clients/${encodeURIComponent(siteId)}/fields/${encodeURIComponent(button.dataset.fieldLabel)}`, {
  ...
  $$("[data-field-move]").forEach((button) => {
    button.onclick = async () => {
      ...
      try {
        await adminApi(`/clients/${encodeURIComponent(siteId)}/fields/reorder`, {
