...
/**
 * Reads one order from the shop.
 *
 * Used by the order detail screen to show the live state rather than the last
 * thing the site told us, and by the status update to confirm the result.
 */
export async function getOrder(siteId, orderId) {
  const connection = await loadConnection(siteId);
  const { data } = await request(connection, { path: `wp-json/wc/v3/orders/${Number(orderId)}` });
  return data;
}

/**
 * Changes an order's status in WooCommerce and returns the order as it now is.
 *
 * The return value is the point. WooCommerce applies its own rules to a status
 * change — a refund workflow may land somewhere other than where it was
 * pushed — so the caller is given what the shop settled on, not what it asked
 * for. Reporting the requested status as though it were the result is how an
 * operator believes twenty orders were completed that never were.
 */
export async function updateOrderStatus(siteId, orderId, status) {
  const connection = await loadConnection(siteId);
  const { data } = await request(connection, {
    method: "PUT",
    path: `wp-json/wc/v3/orders/${Number(orderId)}`,
    body: { status: String(status || "").replace(/^wc-/, "") },
  });
  return data;
}

/** One page of the shop's orders, for reconciling the local snapshot. */
export async function listOrders(siteId, { page = 1, perPage = 20, status = "" } = {}) {
  const connection = await loadConnection(siteId);
  const { data, headers } = await request(connection, {
    path: "wp-json/wc/v3/orders",
    query: { page, per_page: perPage, status: status || undefined, orderby: "date", order: "desc" },
  });
  return { orders: Array.isArray(data) ? data : [], total: Number(headers?.get?.("x-wp-total") || 0) };
}
