...
## 2.4.15 — WooCommerce orders end to end, Mini App order controls

- Completes the WooCommerce integration that previously had no destination: order webhooks were rejected with `Missing post_id`, so enabling order forwarding in WordPress did nothing.
- Adds `POST /webhooks/woocommerce` and accepts `woocommerce.order.*` / `order.*` events on the existing `/webhook/ad` endpoint, ahead of (and without touching) the advert `post_id` guard.
- Stores an order snapshot, notifies the site's owner and admins by Telegram direct message with a Mini App button, and deduplicates by a unique index so concurrent deliveries notify exactly once.
- Adds a real status write-back: the Mini App pushes to WooCommerce `wc/v3` (or the Jarchi plugin route as a fallback) and reports the status WordPress confirmed, never the one requested. Unreachable, refused and unconfirmed responses are surfaced as failures.
- Enforces site-level authorization server-side on every order endpoint; an order id belonging to another site answers `404`.
- Adds `site_order_settings` so each site chooses which order fields appear in its notifications; address and email default to off.
- Mini App: Orders tab for owners and admins, publication fields editable by site owners without operator rights, and an explicit "پیشنهاد جارچی" publication preset.
- Fixes the account router guarding the whole `/api` tree: mounted at `/api` without a path, its customer authentication answered every `/api` request — `/api/admin/auth/login` included — with a customer 401, so the Super Admin API was unreachable from a browser. It now guards only `/api/account`.
- Rebuilds `app-bale-legacy.js` from the current `app.js`, adds `scripts/build-bale-legacy.mjs`, and tests that the two cannot drift.
- Adds migration `0018_woocommerce_orders.sql` (additive) and fixes the `package.json` / `package-lock.json` version inconsistency.
